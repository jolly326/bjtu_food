...
/**
 * 食堂含档口树（食堂 → stalls[]）——**2026-09-21 §7.33 端点合并**：原 `GET /canteens/all`
 * 已删除，改由 `GET /canteens?include=stalls` 提供（映射逻辑不变）。
 * P2-11 / PR-12：返回定型为最小 DTO `CanteenWithStalls`（消费方只读 `id` / `name` 与 `stalls[].id` / `name`），
 * 消除此前 `RawRow[]`（=`Record<string, any>[]`）在反馈页位置选择器上的 `any` 逃逸；
 * 后端多余字段不映射、不透传（含档口树的出参本身也已收敛为 id / name）。归一化边界说明见 shared.ts。
 */
export async function getCanteensWithStalls(): Promise<CanteenWithStalls[]> {
  const rows = await get<RawRow[]>('/canteens', { include: 'stalls' })
