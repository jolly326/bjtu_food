...
const pwdStrength = computed(() => {
  ...
  if (!pwd) return { label: '', color: 'var(--border-strong)', percent: 0 }
  let score = 0
  ...
  const levels = [
    ...
    { label: '强', color: 'var(--color-success)' },
...
<template>
  <PageContainer>
    <PageHeader title="账号设置" subtitle="管理你的个人资料与登录安全" />

    <div v-if="loading" class="empty-card">正在加载账号信息...</div>
    <div v-else-if="!currentUser" class="empty-card">账号信息加载失败，请重新登录后再试</div>
    ...
    <template v-else>
      ...
      <PageSection title="安全设置">
        <template #header-extra>
          <div class="panel-actions">
            <button class="btn-primary btn-sm" v-press @click="openPwdModal">修改密码</button>
          </div>
        </template>
        <div class="detail-body">
    ...
    <FormDialog :show="showPwdModal" title="修改密码" :width="420" confirm-text="确认修改" @close="showPwdModal = false" @confirm="changePassword">
      <div class="pwd-field">
        <label>当前密码</label>
...
<style scoped>
.empty-card { padding: var(--space-4); color: var(--text-muted); text-align: center; }
...
.pwd-field label { display: block; margin-bottom: var(--space-2); font-size: var(--font-sm); color: var(--text-secondary); font-weight: var(--weight-medium); }
.pwd-field input { width: 100%; padding: var(--space-2) var(--space-3); border: 1px solid var(--border-strong); border-radius: var(--radius); font-size: var(--font-base); outline: none; box-sizing: border-box; background: var(--bg-card); transition: border-color .2s var(--ease-out), box-shadow .2s var(--ease-out); }
.pwd-field input:focus { border-color: var(--color-primary); box-shadow: 0 0 0 2px color-mix(in srgb, var(--color-primary) 15%, transparent); }
.pwd-strength { display: flex; align-items: center; gap: var(--space-2); margin-top: var(--space-2); }
.strength-bar { flex: 1; height: 4px; background: var(--bg-gray); border-radius: var(--radius-sm); overflow: hidden; }
.strength-fill { height: 100%; border-radius: var(--radius-sm); transition: width .3s var(--ease-out), background .3s var(--ease-out); }
.strength-label { font-size: var(--font-sm); font-weight: var(--weight-medium); width: 40px; text-align: right; }
.pwd-match-hint { font-size: var(--font-sm); margin-top: var(--space-1); display: block; }
