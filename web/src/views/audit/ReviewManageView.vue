 ...
 * 「评价」（一级导航同名）= **事后处置**页 —— 列表 / 隐藏 / 显示 / 删除 / 批量 / 配图查看 / 详情抽屉。
 * 2026-09-15（本轮精简）：页内统计行（statItems / loadStats，2 次额外 total 请求）删除，
 * 数量只由表格 footer 承担；`?rid=` 深链定位、显隐开关、批量与详情能力全部保留。
 *
 * 为何不再有「安检复核」：内容安全检测已改为 pass/review 均放行、仅 risky 拒绝，
...
<template>
  <PageContainer>
    <!-- 页头：H1 与一级导航同名；副标题只留必要的一句说明 -->
    <PageHeader title="评价" subtitle="隐藏 / 删除不当内容" />
    ...
    <FilterBar v-model="searchQuery">
      ...
      <template #actions>
        ...
        <!-- 数量由表格 footer「共 N 条」承担，此处不再重复展示 -->
      </template>
    </FilterBar>
    ...
    >
      ...
      <!-- 内容只作文本展示（截断 + 悬停看全文）；动作统一收敛到右侧操作列，避免「内容看似可点」的隐式入口 -->
      ...
      <template #actions="{ row }">
        ...
        <button class="link danger" v-press type="button" @click="removeReview(row)">
          <el-icon class="act-ico"><Delete /></el-icon>删除
        </button>
      </template>
    </DataTable>

    <!-- 评价详情（只读，公共组件 ReviewDetailDialog：与菜品详情页共用唯一实现；动作只留在列表行内） -->
    <ReviewDetailDialog
      :show="!!detailReview"
      :review="detailReview"
      :user-name="detailUserName"
      :dish-name="detailDishName"
      @close="closeDetail"
    />
  </PageContainer>
</template>
