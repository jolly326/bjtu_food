...
## 已关闭（Closed）· 图片显示不一致专项（2026-09-14）

### Q-033 · P0 · 同一图片「首页不显示、详情页正常」（根因：缩略图旧约定误改 COS 绝对地址）
- 提出：用户报障｜2026-09-14｜类型：T4 实现约束冲突
- **根因**：`utils/image.ts` 的 `getThumbUrl()` 按**本地磁盘存储**时代的约定把 `xxx.jpg` 改写为 `xxx_thumb.jpg`。图片迁到 COS 后，COS 上不存在 `_thumb` 变体文件，于是列表请求 `..._thumb.jpg` **返回 404 → 显示占位图**；详情页走 `getImageUrl()` 原图，故正常。
- **复现证据**：`原图 HTTP 200（166,847 字节）` vs `..._thumb.jpg HTTP 404`
- **受影响位置**：`pages/home/DishCard.vue`（首页瀑布流）、`pages/find/FindResults.vue`（搜索结果）；均已修复
- **修复**：`getThumbUrl` 区分两条链路——
  - 云存储绝对 URL（`*.myqcloud.com`）→ 追加 COS 图片处理参数 `?imageMogr2/thumbnail/400x` 生成**真缩略图**（实测 166KB → **37KB**）
  - 本地磁盘相对路径（`/images/...`）→ 保留 `_thumb` 推导（本地开发/历史数据）
  - `cloud://`、`data:`/`blob:`、非 COS 域名、已含处理参数 → 一律原样返回
- **验证**：逻辑边界 7 例全部符合预期；`type-check` EXIT=0；`build:mp-weixin` Build complete
- 状态：`closed` ✅（待部署生效）

### 图片链路一致性审计结论（同批）
| 位置 | 取图路径 | 修复前 | 修复后 |
|---|---|---|---|
| 首页瀑布流 | `getImageUrl(getThumbUrl())` | ❌ 404 白图 | ✅ COS 400px 缩略图 |
| 搜索结果 | 同上 | ❌ 404 白图 | ✅ COS 400px 缩略图 |
| 详情大图 | `getImageUrl()` 原图 | ✅ | ✅ 原图（不变，清晰度优先） |
| 评价配图 / 头像 | `getImageUrl()` | ✅ | ✅ |
| 反馈选菜 / 纠错缩略图 | API 层 `normalizeImages` 已归一化（原图） | ✅ 可显示（但加载原图） | ✅（**建议**后续改走 `getThumbUrl` 省流量，本轮未改以避免范围蔓延） |
| 管理后台列表/预览 | `toAbsoluteImageUrl()`（无 `_thumb` 逻辑） | ✅ | ✅ |

> **已知有意差异（需产品确认是否接受）**：列表用 400px 缩略图、详情用原图，属既定的性能取舍（原 C14 设计意图）。若要求两处**像素级一致**，可让列表也用原图（改一行，但列表流量与弱网时延上升）。
...
## 已关闭（Closed）· 第 6~7 批（2026-09-14）

| ID | 问题 | 决策 | 落地 |
|---|---|---|---|
| Q-013 | 「找吃的」之外的新入口 | **全不做**；**热搜词保留**（已澄清：不新增 ≠ 删热搜词） | 零改动（现有即符合） |
| Q-014 | 下架菜品的评价可见性 | **保留可查** | 零改动（核实 `selectReviewPageByUserId` 未按下架过滤，现状即符合，已写入 §7.12 第 3 条） |
| Q-015 | 举报超时与重复举报 | 超时**不自动处置**（仅后台置顶提醒）；**同一人重复举报去重** | 去重已实现（`FeedbackServiceImpl` 提交前查重；游客不去重，边界登记） |
| Q-016 | 只读管理员口令 | **不做**，维持单口令 | 零改动（§7.12 第 2 条） |
| Q-017 | 上线验收标准 | **三项并行**：构建门禁 + 手工走查 + 线上自动冒烟 | 已产出 `docs/loop/launch-checklist.md`（§7.12 第 1 条） |
| Q-018 | 31 道菜数据校对 | **逐条校对** | 已产出 `docs/loop/dish-proofread-checklist.md`（含 31 条现状 + 全缺图 + id6/24 疑似重复） |
| Q-027 | 分页契约收口 | **后端补齐** `records/page/pageSize`（保留 `list` 过渡） | 已实现（10 个文件，`list` 为只读派生恒等值） |
| Q-028 | 注销确认色 | **保留主色**，仅登记常量 | 已实现（`MODAL_CONFIRM_PRIMARY_COLOR` 入 tokens，裸 hex 清零） |

> **注**：本文件已收敛为「**同一问题 ID 只有一份现行真值**」。Q-013~Q-018、Q-023~Q-028 的历史登记原文均已移至文末「**历史登记（已闭环）**」附录，其最终闭环结论以上方各 Closed 段为准。当前真正开放项为 **Q-004、Q-005、Q-006、Q-007**（Q-008 已闭环）。
...
- 补充（2026-09-15 备注）：STATUS.md 记 D1 部署完成后 `ADMIN_TOKEN` 已生效（launch-checklist B3/B4 实测 200/403），待正式复测后闭环
...
> 其余历史登记原文（Q-008 原文、Q-013~Q-018、Q-023~Q-028）见文末附录「历史登记（已闭环）」。
...
### Q-008 · P2 · 「喜欢 / 有用」计数口径未定型 → 已闭环（2026-09-14）
- **决策**：明确**不做**独立「喜欢」计数（与「收藏已移除」口径一致）；仅保留评价「有用」点赞
- **落地**：`DishServiceImpl` 误导性悬空注释已清理（A 批次）；全仓悬空待办注释清零
- 状态：`closed` ✅
...
> 以下为历史登记**原文**，仅作审计留痕。每个 ID 的现行真值（最终闭环结论）以本文档各 Closed 段为准，本附录内容**不再更新**。

### Q-008（历史登记原文） · P2 · 「喜欢 / 有用」计数口径未定型
- 提出：主 agent｜2026-09-14｜类型：T1
...
### Q-023（历史登记原文） · P1 · 管理端口令体系下是否还需要「操作人身份」
...
### Q-024（历史登记原文） · P2 · 用户菜单「账号设置」入口去留
...
### Q-026（历史登记原文） · P1 · 上新 / 促销 / 新晋黑马 / 猜你喜欢 / 档口菜品 板块是否保留
...
### Q-013（历史登记原文） · P1 · 首版功能边界：「找吃的」之外还要哪些入口
...
### Q-015（历史登记原文） · P1 · 举报「48 小时」承诺的异常兜底
...
### Q-018（历史登记原文） · P2 · 现有 31 道菜的数据校对责任
