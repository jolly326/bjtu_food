...
- ~~**`list` 为过渡期历史字段**（只读派生自 `records`，二者恒等值）~~ **已于 2026-09-21 删除（change `api-slimming`）**：删除依据——① 两个消费端均**优先读 `records`**（client `api/shared.ts` 的 `recordsOf`；web `api/adapter.ts` 的 `pageRecords`），服务端恒返回 `records` → `list` 分支永不命中，**全端零读取**；② `getList()` 恒等于 `records` 且参与序列化 → **同一数组被 JSON 输出两次、列表响应体积≈翻倍**。**影响面**：`PageResult` 为全部列表接口共用（`/dishes`、`/my/reviews`、`/my/notifications`、`/admin/*` 等），已**一次全端切换、无半迁移态**。**状态：已对齐（2026-09-21 落地）**——`getList()` 与 `@JsonPropertyOrder` 的 `list` 已删除；两端 `recordsOf` / `pageRecords` 的 `list` 兜底同批移除；分页壳只输出 `records` / `total` / `page` / `pageSize`。
- **注记（2026-09 核实，2026-09-14 / 2026-09-21 更新）**：此前记载的 `PageResult{ list, total }`（仅两字段、无 `page/pageSize`）形态**已过期作废**——所有经 `common/result/PageResult` 归一的接口（`GET /my/reviews`、`/admin/feedbacks*`、`/admin/dishes`、`/admin/users`、`/admin/reviews` 等）现均为 **`records` / `total` / `page` / `pageSize`** 统一 4 参形态。
...
| GET | `/dishes` | `DishQueryReq`（keyword/canteenId/**mealType**/minPrice/maxPrice/page/pageSize） | `PageResult<DishVO>` | 菜品分页搜索 / 筛选（keyword 同时命中**菜名 / 别名 alias / 档口名 / 食堂名**四处模糊匹配）。**（2026-09-21 决议：删 `stallId` / `sortBy` / `sortOrder` 三个参数——`stallId` 端上零发送（筛选条只有食堂 / 价格）、`sortBy`/`sortOrder` 端上零消费（仅 `heat` 被使用且 `heat` 分支固定 `DESC`，`sortOrder` 对任何取值均无效果）；排序口径收敛为服务端恒热度，消除「首页传 heat、筛选 / 搜索不传而落 `rating_count` 分支」的口径分裂。**状态：已对齐（2026-09-21 落地，change `api-slimming`）**——三参数与 `DishMapper.xml` 的排序 `<choose>` 均已删除，查询恒按 `heatScoreExpr` 倒序（实测：默认流 / `?canteenId=` / `?keyword=` 三路均为同一热度序的子序列）。）** **（2026-09-21 新增参数 `mealType`，change `home-ui-refresh`）**：单值大类筛选（`set_meal` / `stir_fry` / `noodle` / `dry_pot` / `snack` / `soup_drink`），**白名单校验、非法值 → 响应体 `code=400`（不静默降级）**，可与 `canteenId` / `keyword` / 价格区间叠加且不改变排序口径。**状态：已对齐（2026-09-21 落地）**，实测六类 `total` = 3/9/4/3/9/3、与 `canteenId` 叠加生效、`mealType=foobar` 返回 `code=400` |
...
| **账号出参精简**（2026-09-21 用户拍板，spec §7.32） | ① 小程序：`POST /auth/wechat-login`、`POST /auth/verify-email` 与 `GET\|PUT /auth/profile` 的账号信息对象删 `email`（恒 NULL 死字段）、`guestShortId`（纯派生）、`status`（端上零消费）；校园邮箱唯一出参来源收敛为 `bindEmail`。② 管理端：`UserVO` 删 `guestShortId`（同判据——`id` 的纯派生值；`status` / `email` 保留）；`GET /admin/users` 响应不再含该字段 | **已对齐（2026-09-21 落地，change `auth-response-slimming`）**：`UserInfoVO` 恰 6 字段、`UserVO` 去 `guestShortId`；`toUserInfo` 与 `buildProfileMap` 两处承载结构同步收敛（`AuthServiceImpl` 内 `buildGuestShortId` 与三字段读写零残留）；建号抽为 `UserService.createWechatGuest`（独立 Bean 事务方法，远程 `code2Session` 在事务外）；client `types/user.ts` / `api/user.ts`（去 `email` 映射与账号信息推导）/ `pages/profile`（读 `bindEmail`）/ `pages/mine`（短标识按 `id` 现算 + 本地兜底）已对齐；web `types/index.ts` / `api/adapter.ts` 去字段声明与映射，`UserView` 过滤与兜底展示改由 `id` 现算。三端门禁：`mvn compile` / `client type-check` / `web build` 均 EXIT 0 | **已对齐** |
