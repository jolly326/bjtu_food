...
- 认证：JWT 经 `JwtAuthFilter`；白名单（实际 `SecurityConfig`，同路径已含 `/api` 前缀）为任意方法放行：`/auth/wechat-login`、`/auth/email-code`、`/auth/verify-email`、`/feedback`（公开提交）；`GET` 仅放行公开浏览：`/dishes/**`、`/canteens/**`、`/reviews`、静态图片 `/images/**`（**2026-09-15 CT-05 对齐 `SecurityConfig` 现状**：`/stalls/**` 幽灵白名单已删除（无公开 StallController 端点）、**`/categories` 已从公开白名单移除（DOC-01）——品类管理仅经 `/admin/categories`，公开品类端点不存在**）；学生 UGC 写操作需 `verified=true`（见 §5 认证与鉴权），不再依赖 `STUDENT` 角色。**`/admin/**` 不由 JWT/角色把关**（2026-09-14 与 §7.10 对齐，原方案 C 描述作废）：`SecurityConfig` 放行后由 `AdminTokenFilter` 以环境变量口令（`X-Admin-Token`）校验，未配置 fail-closed 403；**无 `/auth/admin/login` 白名单项**（该端点不存在）。**移除 `/auth/login`、`/auth/register`、`/auth/password/reset`（废除账号密码登录）**；`PUT /auth/password` **已删除**（2026-09-14 Q-108 用户确认：端点 / Service / DTO / 端上声明全部移除，学生端无密码体系，见 §5.y.1 与 §7.21 第 3 条）。
...
1. **Q-117 品类保留为后台归类用途、端上不呈现（用户已确认）**：后台**保留**品类（`category`）作为**菜品归类**用途——仅供管理员自行组织菜品（菜品表单可填 `categoryId`）；**端上不呈现**品类、**不作为学生筛选维度**。
   ...
   - **落地口径（2026-09-15 CT-05 对齐修订）**：**公开端点 `GET /categories` 已删除**（三端零调用，公开白名单不含，见 §3），品类 CRUD 仅经 **`/admin/categories`**；`DishQueryReq.categoryId` 保留为后台归类查询维度，**端上不传**（端上无品类筛选入口）。本项与 PR-01（有录入入口且能落库才可放出维度）一致——品类有后台录入、可落库，但**不放出到端上**，故不构成假功能。
