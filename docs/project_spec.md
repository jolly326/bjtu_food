...
- `ADMIN`（系统管理员 / 食堂后勤）：审核 UGC、配 Banner、看板、食堂 / 档口 / 菜品 CRUD + 上架下架、用户 / 管理员管理、**录入活动（活动标题 / 描述 / 发布时间 / 公众号文章链接，手动录入，不经学生 UGC、不经审核流）**。**管理后台登录沿用方案 C：管理员账号密码 + BCrypt + JWT（放弃微信开放平台扫码 / 复用小程序码）**，见 §1 认证与 §5 认证红线。
...
- **学生 UGC 路径**：提交档口 / 食堂仅 `POST /my/stalls`，发布菜品仅 `POST /dishes` 系列，写评价 / 评论 / 点赞 / 动态等社区写操作——均需 `verified=true`（见 §5.y 权限矩阵）；严禁 `/stall-owner/**`。
