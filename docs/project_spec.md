...
- ~~dish.alias 别名搜索本期落地~~ **2026-09-21 拍板：`dish.alias` 字段整体删除**（不再需要别名层，关键词直接硬匹配菜名 / 档口名 / 食堂名；见 §7.35，**代码已于 2026-09-22 落地——change `search-page-refresh`**，存量远程库列待用户执行 `schema.sql` 幂等段）。
...
- **Web 端管理能力全景（2026-09-15 IA 扁平化重写：一级导航 4 项，路由与导航 1:1）**：
  ...
  - **~~系统 / 操作日志~~ 已删除（2026-09-15 用户拍板，见 §7.25 第 1 条）**：`OperationLogView` 页面与 `GET /admin/operation-logs` 端点、`operation_log` 表全链移除；**管理端不存在操作日志页，亦不存在任何操作留痕能力**（`ClientIpUtil` 仅服务 `RequestLoggingFilter` 服务端访问日志；`view_log` 浏览足迹**仅作浏览计数来源**（~~浏览量当日去重判据~~ 已随 §7.41 于 2026-09-23 作废）（**「猜你喜欢」已随 §7.10 第 1 条下线，不再是其消费方**），与管理员留痕无关，均不属本项）。
...
3. **用户 UGC —— 评价类**：**菜品评价是唯一的「评价」形态**（社区/动态下线后不再有第二种评价语义载体）。菜品详情页评价区（展示 + 内联加载）+ 底栏「写评价」提交；一人一菜一评（`uk_review_user_dish`），并**支持覆盖式「重新评价」**（`PUT /reviews/{id}`，真源见 `docs/feature/client-写评价.md`）。**评价支持配图（≤3 张）**，文本与图片均须过微信内容安检（内容安全检测 `pass` / `review` 一律放行、`risky` 拦截，**无人工复核**，见 §5.a 与 §7.24）。~~评价侧「有用」点赞一人一票（`uk_useful_user_review`）~~ **已于 2026-09-20 全链下线**（端点 / `ReviewVO.usefulCount` / `review_useful` 表 / `review.useful_count` 列 / 端上按钮 / Web 列全部删除，见 §7.30 清单 #1）。
...
- **图标（SVG 矢量）**：按 §4.2 映射（**内联注册于 `IconSvg` 的 `ICONS` 键表**，无本地 `assets/icons` 目录；**兜底渲染 `empty` 中性占位键**）；新增语义须在 `ICONS` 登记图标键，禁止 emoji 字符当图标，不得私自引入未登记图标。**`thumb`（有用/点赞）键与 `--color-like` token 已随「评价有用」全链下线删除（2026-09-21）**；`heart` 键已删除，无「喜欢」语义图标；各键语义互不混用。
...
- **UI 全量审计红线（2026-08-02 补充，BLOCKER 级，违反即阻断）**：以下规则自 2026-08-02 全量审计结果提炼，**与上方四条红线同属强制，新增/整改页面不得回退**：
  ...
  - **`IconSvg` 的 `color` SHALL NOT 传 `var(...)`（2026-09-23 新增，QA 门禁发现）**：图标经 **SVG data-uri** 渲染，颜色在 data-uri 字符串内为**字面量硬编码** —— 传 `var(--x)` 时 `IconSvg` 的取色逻辑（**现行 `stroke` computed**；原 `resolveColor` 已随 MP-11 删除）会**恒回落 `ICON_FALLBACK_COLOR`（`#1C1C1E` 近黑）**，表现为「**grep 看似合规、渲染全黑**」的**假阳性**（该缺陷曾以「已按 §7.39 改 `var(--color-star)`」的形式假性通过评审）。引用 token 时须传 **`COLOR_MAP['x']` 实色**（唯一真源 `client/src/theme/tokens.ts`）。**grep 期望 0 处**（范围限 `client/`，且仅约束 `IconSvg` 的 `color` 传参语境）。**类型层兜底**：组件若接受图标色入参，其类型应限定为 **`(typeof COLOR_MAP)[keyof typeof COLOR_MAP]`**（如 `pages/detail/dish/ActionSheet.vue` 的 `ActionSheetItem.iconColor`），使传 `var(...)` 在**编译期**报错 —— 以编译器替代 grep 兜回归。**例外（不受本条约束）**：CSS 属性（`background` / `color` / `border` / `box-shadow` 等）中的 `var()` 由 WXSS / 浏览器正常解析，照常使用；`<swiper indicator-*-color>` 的原生属性例外见上条。
...
- **认证**：微信打开静默登录（`wechat-login`）即游客态；UGC 写操作需 `verified=true`（邮箱验证码认证）；**废除账号密码 / 注册**（管理后台亦无登录体系，改为环境变量口令，见 §5.y.5）。评价一人一菜一条（`uk_review_user_dish`）——**该唯一键同时是「重新评价走覆盖更新」的并发保障**（`PUT /reviews/{id}` 覆盖同一行、无删行重建）；~~点赞一人一票（`uk_useful_user_review`）~~ **已随「评价有用」全链下线（2026-09-20）**，业务代码须与唯一键现状一致。
...
- **权限矩阵**：
  ...
  - **系统通知（`/my/notifications/*`）→ 服务端认证专属（`verified=true`）、前端游客可直达（2026-09 校准，见归档 capability `openspec/specs/client-auth-boundary`）**：通知是按 `userId` 归属的账号私有数据（`/my/` 前缀；服务端 `@RequireVerified` 校验，游客请求被拒），内容为内容贡献者的行为反馈（审核结果）；游客（`verified=false`）无任何可产生通知的 UGC 写操作来源，个人通知恒空，**不属公开数据**。**前端边界（替代原 `authLocked=true` 入口锁定口径，前端已无该机制）**：「我的」页「系统通知」入口对游客**直接放行**进入通知页，页面**不挂 `AuthSheet`、不弹认证引导**；游客无个人通知或请求被拒时**静默处理**（不呈现空态提示 / 错误态 / 认证引导，「暂无通知」轻提示仅 `verified=true` 展示，失败静默，恢复走「加载失败 · 点击重试」块（MP-012）或重入页面（`onShow`）重拉——**页面级下拉刷新已于 2026-09-22 整体取消，见 §7.37**）；游客态**不拉取未读数**（红点仅在 `verified=true` 时刷新）。本条**不适用**下方「入口不置灰、点击弹认证」口径。
...
- **D-B** `view_log` 记录浏览足迹：当前（2026-08-19）采用**应用层 upsert**（`HistoryService.recordDishView` 存在则更新 `updated_at`、不存在则插入），未加唯一键（`uk_view_user_target` 为可选增强，待需严格去重时再补 DDL）。`addViewCount` 时同步写入足迹。**（2026-09-23：浏览量去重取消后，`view_log` **不再参与计数逻辑**、仅作浏览足迹保留；upsert 语义与「游客不写足迹」不变 → 本条技术口径**继续有效**，仅其「当日去重判据」用途作废，见 §7.41。）**
- **D-C** ~~报表导出返回 CSV 文件流，不引 Apache POI~~（报表导出功能已随 `ReportExportView` 清理移除，2026-08-18；此决策作废，留档备查）。
- **D-D**（2026-09-15 按 §7.10 第 1 条收窄）热门 / 推荐 / 猜你喜欢 / 新晋黑马等高频读接口已随端上零消费整体删除，**~~Caffeine 短 TTL 缓存(60s) + 写失效现仅剩热搜词条 `GET /dishes/hot-search`~~（`CacheConfig.CACHE_DISH_HOT_SEARCH`，`@CacheEvict` 写失效）**；原 `recommendDishes()` 已删除。**（2026-09-22 修订，change `search-page-refresh`：该端点改 `GET /dishes/for-you` 且去缓存，Caffeine 缓存设施整包退役——**服务端当前无任何进程内读缓存**；见 §7.35 第 2 条与本页 §8「搜索契约精简批」③。**本条决策本身不变**：不引 MQ / 不引入新缓存中间件，高频读接口零消费即删；恢复读缓存须重新论证并登记。）**
...
- **喜欢 / 收藏单一概念（收藏全量移除，2026-08-12 复核）**：原 `favorite`/`/favorites` 端点、表、字段（`favoriteCount`、`isFavorited`）已彻底删除；**前端不得保留任何「收藏」入口或按钮**（含 `pages/mine/index.vue` 的「我的收藏」、`pages/detail/dish/index.vue` 底部收藏按钮、`my-favorites` 页），统一移除。**原 `thumb`（有用/点赞）键已随「评价有用」全链下线删除（2026-09-21 资产专项）**（`heart` 键亦已删除、无「喜欢」语义图标，见 §4.2 S4-05）；禁止 `like`/`favorite` 双体系、禁止 `like_count`。`DishVO` 不再含 `favoriteCount`/`isFavorited`（历史口径混淆已废）。
- **状态枚举**：Dish `status` on/off（**`audit_status` 列与索引已于 2026-09-15 阶段4 全量删除，见 §7.23 第 4 条：菜品无独立审核**）；**Canteen/Stall 无 `status` 枚举**（停业/实体审核语义已随去实体化下线，2026-09-14 Q-119，见 §7.22 第 3 条）；Web 内部 `active/inactive` 须经 adapter 映射回后端枚举（仅用于菜品）。（~~Banner~~ 及 Broadcast/Activity 已随公告/活动下线移除；**⚠️ 2026-09-23 更正：Banner 已于 2026-09-22 以「首页顶部 16:10 多图轮播」形态重新引入**（`GET /banners` + `banner` 表，见 §7.42）—— 原「Banner 已移除」的表述就此作废。）
...
- 载体字段已具备：`FeedbackReq` 的 `type` / `content` / `images`（≤3 张 COS 绝对地址）/ `relatedType` / `relatedId`（不收集联系方式，无 `contact` 字段）。
...
- **二期**：投稿仅作管理员**参考信息**，须管理员人工编辑后录入菜品，不得直接生成菜品数据（见 §7.13 第 1 条）。

### 7.4 UGC 审核：先发后审 + 举报，纯人工巡查
- 评价与配图**实时可见**（先发后审），**不做**举报自动隐藏阈值。
- 举报链路已闭环：`feedback(type=report)` + `relatedType=review` + `relatedId`；小程序端 `ReportModal` 挂菜品详情页评价卡的「**举报评价**」入口（唯一挂载点）；管理端 `FeedbackAdminController` 可按 `type` 筛选。（本条其余结论不变）
- 处置手段均已具备，**无需新增开发**（**均为事后处置**）：`PUT /admin/reviews/{id}/hide`（隐藏 / 显示）、`DELETE /admin/reviews/{id}`（删除）。
- 运营要求：管理员须定期巡查反馈与评价列表，无自动化兜底。
- **机制澄清**：评价侧**无 `audit_status` 字段、无人工前置审核态、亦无人工复核态**。`review` 表以 `is_hidden`（默认 `0`）为唯一可见性开关。「先发后审」的实际机制为：内容安全检测 `pass` / `review` 一律 → 立即对外可见（`is_hidden=0`）；内容安全检测 `risky` → 提交时即被 `checkText` 抛 400 拦截、不落库。管理员事后通过 `PUT /admin/reviews/{id}/hide`、`DELETE /admin/reviews/{id}` 处置。
...
- 依据（2026-09-14 代码核实）：
  - 登录方式仅有 `POST /auth/wechat-login`（有 openid）与 `POST /auth/email-code`（无 openid）；**无学号密码登录**（`user` 表无密码字段）。
...
> **准入范围收窄（见 §7.23 第 2 条）**：本双约束**仅作用于 `POST /dishes/{id}/reviews`（写评价正文与配图）**——准入口径以 §5.y.4 为准。

### 7.6 收藏：不存在，且不做
- 产品确认**无收藏功能**，后续也不做；文档与需求表述一律不得将「收藏」作为功能项。

### 7.7 落地任务分档
**上线前必做**
1. UGC 准入双约束**仅作用于评价（含评价配图）**：写评价 / 评价传图须 `verified=1 && openid != null`。**准入失败按原因分码（以实现为准）**：`verified != 1` → **`4031`**（`@RequireVerified`/`RequireVerifiedAspect` 触发，message「请先完成学号邮箱认证」，端上弹认证引导 `AuthSheet`）；`verified=1` 但 `openid` 为空 → **`403`**（message「请使用微信登录后再发布评价」，端上提示微信登录，**不弹邮箱认证引导以免误导**，见 `ReviewServiceImpl`）。两种情形的用户引导动作不同，故有意分码。
   **反馈（含投稿 `type=add`、纠错 `type=error`、举报 `type=report`）不设认证门槛**，保持公开免认证 —— 否则会把 §7.3 的学生投稿通道掐死（新用户未认证即无法投稿，与该条决议自相矛盾）。
   反馈虽免认证，服务端内容安全检测照常执行：有 openid 时正常调 `msgSecCheck v2`（`risky` 拦 `400`，`pass` / `review` 放行）；无 openid 时按既有边界**跳过内容安全检测放行**（`msgSecCheck v2` 必填 openid、无法调用）。反馈内容**不对外公开展示**（仅管理员在后台可见），故该边界暴露面可接受。无人工复核与复核队列动作（见 §7.24）。
   **端上处置口径：403 缺 openid 只「提示 + 用户主动确认」，禁止自动重登换登录态（2026-09-14 技术负责人裁决，QA 门禁提出）**——小程序端收到 `403` 且 message 指向「微信登录」（`verified=1` 但 `openid` 空）时，端上处置为**弹窗呈示后端 message（如「请使用微信登录后再发布评价」）+ 确认按钮，由用户主动触发重新微信登录**（用户确认后才执行 `forceLogout` + 完整静默登录补 openid）；**禁止无用户确认的自动换号**。理由：① **授权边界**：本条 / Q-111 / §3 的授权词均为「**提示**微信登录」，未授权端上代用户执行账号身份变更；按 PR-02，鉴权与错误码的处置口径权威在后端与本文档，端上只消费不覆写。② **静默登录场景错位**：静默建号的授权场景是「首次进入的无感建号」（§5.y.3，用户无旧身份，无感知零成本）；本场景用户持有**已认证账号**，自动重登按新 openid 建新游客号，登录态（含 verified）被用户无感知互换，违背「身份变更须用户可知可控」。③ **顺滑收益为假**：自动重登所得新游客号 `verified=false`，用户重试发评价仍被 `4031` 拦截、仍须走邮箱认证（经 §5.y.3 凭原邮箱合并找回原账号）——认证一步未省，收益≈0，而静默换号的用户困惑代价真实存在，风险收益不对称。④ 该场景系 email-code 链路账号（极低频），弹窗确认的摩擦成本可接受。本裁决不改变 401 处置（静默登录恢复，§5.x）与 4031 处置（弹认证引导）。
2. 反馈入口文案：投稿类目标注「提交后由管理员处理，确认后才会展示」。

**二期**
3. 管理端反馈列表**不做「一键转菜品」**：投稿仅作管理员参考信息，须人工编辑后录入，不得直接生成菜品数据（见 §7.13 第 1 条）。

### 7.8 第二轮对齐决议（2026-09-14 用户拍板）

1. **管理端菜品直接生效**：管理员在后台**新增/编辑**菜品时**直接生效、无需审核环节**（管理员即权威）；**菜品无独立审核**，公开可见性唯一判据为 `dish.status='on'`（见 §7.23 第 4 条），学生投稿（反馈 `add`）由管理员**在反馈处理中人工编辑后录入菜品**。
2. **投稿形态**：学生投稿新菜品沿用**现有反馈表单**（自由文本 + 配图 ≤3 张），**不新增结构化字段**；由管理员人工解析后在菜品管理页录入。`type=add` 为新增菜品、`type=error` 为信息纠错。
3. **准入边界（见 §7.23 第 2 条）**：§7.5 的双约束**仅约束「产生公开可见内容并需内容安全检测」的评价正文与配图（`POST /dishes/{id}/reviews`）**，不作用于其它入口。
...
1. **`dish.region`（风味/菜系）** → 由 §7.28 取代（描述维度替换，`region` 字段不存在）。
...
6. **`dish.tags` 值域** → 由 §7.29 取代（`tags` 字段不存在，无标签筛选）。

### 7.10 板块与身份降级决议（2026-09-14 用户拍板 · 第 5 批）

1. **小程序端 5 个零消费入口不提供**（端上无任何页面/组件消费，2026-09-14 跨端核实）：`GET /dishes/hot`、`GET /dishes/new`、`GET /dishes/promotions`、`GET /dishes/rising`、`GET /dishes/recommend`，以及小程序端「档口菜品列表」取数函数。
   **保留**：`GET /dishes`（首页瀑布流与筛选）、`GET /dishes/{id}`、`POST /dishes/{id}/view`、`GET /dishes/hot-search`（首页热搜在用）。
   说明：折扣表达见 §7.26（`price` + `originalPrice` 两态）。
2. **管理端「操作人身份」降级**：管理端为环境变量共享口令（无登录体系），认定「单口令即单人」，**不追究操作人身份**——`user_feedback` 不记录处理人、无操作日志能力（见 §7.25）；如需身份追溯须重新拍板。「反馈处理人」的可追溯性**明示降级**。
...
1. **不新增「找吃的」之外的入口**：随机推荐（今天吃什么）、榜单、分享**均不做**；**搜索历史与「猜你想搜」保留**——搜索页发现态的搜索历史为**本地存储**（不上传服务端）、可清空与单条删除，猜你想搜同页静态推荐。首页保持现状——顶部搜索框、瀑布流、**热门搜索词（保留）**。
...
4. **上线前菜品数据逐条校对**：现有 31 条种子菜品由用户逐条核对名称/价格/描述/**荤素 / 主料 / 口味 / 冷热**，并补齐首图（2026-09-14 核实：31 条**全部缺首图**）。校对要点（含疑似重复项 id=6 与 id=24）。
...
1. **浏览量口径** → 由 §7.41 取代（PV 口径：不做人员与时间限制，load 一次即 +1；`view_log` 仅作浏览足迹）。
2. **评价排序** → 由 §7.30 清单 #1 取代（评价列表唯一排序 = **时间倒序**（新评价在前），无排序切换；评价列表仅按菜品维度查询，`dishId` 必填）。
3. **被隐藏的评价对作者本人可见并标注**：`/my/reviews` 不过滤 `is_hidden`，作者可看到自己的被隐藏评价并显示「已被隐藏」标注，避免"评价凭空消失"。公开列表仍仅展示未隐藏（`is_hidden=0`）的评价——**安检不构成可见性闸门**（见 §7.24）；端上状态文案 = 「已被隐藏」。
...
3. **评价列表不提供排序切换**：维持「默认按有用数置顶」的单一默认（后端已支持 `sort` 参数，若日后需要「最新 / 最有用」切换须重新拍板）。 **（⚠️ 2026-09-20 起「不提供排序切换」的结论**仍然有效**，但排序口径已改为「**时间倒序**」——`sort` 参数已整体废除，端上与后端均不再持有排序切换；本条仅作历史留痕，见 §7.30 清单 #1 / #2。）**
...
| **PR-12** | **枚举 / 常量展示必须与后端常量表同源；前端不得手写子集或裸英文枚举；跨端 DTO 必须显式定型（平台句柄外禁止 `any`）。**<br>**（2026-09-23 扩展 · 执行机制）**：判定「是否同源」**必须做全端盘点** —— 在 `server/` / `client/` / `web/` **三端同时** `grep` 该枚举 / 常量的**硬编码点**（映射表、选项数组、裸中文字面量），**不得只查当前正在改的那一端**；唯一的合规模式 = **后端下发字典（`{value, label}` 或 `{key, label, order}`），前端零硬编码**（参照 `GET /dishes/meal-types`）。 | P3-09、P3-10、P2-11、P3-12；§7.40 第 4 项（四维三套真源）；§7.34（`mealType` 字典下发的合规先例） | ~~操作日志前端只登记 7/12 动作、单元格显示裸英文~~（**2026-09-15：操作日志能力已整体删除，本案例仅作 PR-12 历史例证，见 §7.25 第 1 条**）；反馈页食堂树用 `any`；同类 VO 存在双份；**四维「荤素 / 主料 / 口味 / 冷热」的映射表在 client 与 web **各硬编码一份**（连同后端常量表共三套真源），且历次评审**只发现了 client 一侧**（见 §7.40 第 4 项）** | 筛选项恒空 / 漏项；字段名变更时端上静默失效且无编译期保护；**只查一端 → 另一端漏网，问题被「搬走而非解决」**。 |
...
- **保留（不属操作日志能力，不得误删）**：`ClientIpUtil`（仅服务 `RequestLoggingFilter` 服务端访问日志）；`view_log`（浏览足迹，**仅作浏览计数来源**~~，当日去重判据已随 §7.41 作废~~）**不动**。
...
- **~~机器值 / 中文映射唯一真源~~在 client `api/dish.ts`（沿用原 `TAG_MAP` 的映射模式），视图层禁止二次映射。** **（⚠️ 2026-09-23 修订：本句口径已推翻 —— 实测该映射被**两端各硬编码一份**（`client/src/api/dish.ts` + `web/src/constants/index.ts`），连同后端常量表共**三套真源**，违反 PR-12。**现行目标形态 = 字典下发 + 多值数组化**，见 §7.40 第 4 项：四维的**值域对应关系**（`meat`→荤 / `hot`→热食 / `pork`→猪 …）仍以本节下表为**真源**，但**映射载体改为后端字典端点**、两端硬编码映射表一并退役。）**
...
| `GET /dishes/hot-search` | ~~`GET /dishes/hot-keywords`~~ → **`GET /dishes/for-you`** | **已拍板取代（2026-09-22，change `search-page-refresh`）**：端点**整体改名 `GET /dishes/for-you`**（旧路径删除、**无兼容别名**），语义由「按热度取热搜词条」改为**随机抽取在售菜品名**（`ORDER BY RAND() LIMIT #{limit}`、**不再按热度排序**）、**响应缓存移除**；原目标形态 `hot-keywords` **不再采用**（语义已不取「热搜词」，该名失去依据，故未落地即作废）。详见 §7.35 第 2 条、本页 §8「搜索契约精简批」②。 |
| `GET /canteens/all` | `GET /canteens?include=stalls` | **已采纳（2026-09-21 落地，change `api-slimming`）**：`/canteens/all` 已删除并合入 `include=stalls` |
...
### 7.32 登录出参精简：删 `email` / `guestShortId` / `status`（2026-09-21 用户拍板）

> 决策来源：用户 2026-09-21 要求对本域做冗余清理后拍板。定性依据（均已核实实现，非推测）：`user.email` 在微信体系下**无任何写入点**（登录与认证只写 `bind_email`；该列仅旧密码体系遗留、注销时置 NULL 释放 `uk_user_email`）→ 出参**恒为 NULL**；`guestShortId` 后端实现即「食客 + `id` 尾 4 位」→ **纯派生**；`status` 端上**零消费**（禁用 / 注销在登录侧 400 与 UGC 写操作侧 403 拦截，端上无分支渲染）。对应 PR-05（冗余边界）。
> 与本章冲突的历史表述（§5.y.4 游客标识、§5.y.5 `GET /auth/profile` 出参清单）**已在本文件就地修订并标注来源**。

#### 1. 口径定型

- **`UserInfoVO`（登录 / profile 出参）删除三字段**：`email`、`guestShortId`、`status`。
- **校园邮箱的唯一出参来源 = `bindEmail`**；端上禁止再以 `email` 或「由 `username` 推导」作为第二来源（PR-02 口径单一真源）。
- **游客短标识改由端上派生**：「食客 + `id` 尾 4 位」；无 `id` 时回退既有的本地游客 ID（`utils/guest.ts`），**不再由接口出参**。
- **`status` 不再对端出参**；账号可用性判定维持服务端权威（登录 400 / 写操作 403）。
- **`guestShortId` 在学生端与管理端**一律不出参**（判据不因端而异：`id` 的纯派生值不作为出参）。管理端 `UserView` 的关键词过滤与昵称兜底展示改为**由 `id` 现算**同一规则，行为等价（2026-09-21 用户拍板：管理端曾"在读"不构成保留理由，可无损替代即删）。
- **管理端 `UserVO` 的 `status` / `email` 保留**（管理端需展示与操作账号状态、展示校园邮箱），本决议不涉及；其 `guestShortId` 按上一条同批删除。
- **`buildGuestShortId` 的规则保留但不再有任何出参消费方**：仍作为建号默认昵称「食客 + ID 尾 4 位」的服务端私有实现。

#### 2. 影响面（文档层已修订；**代码已于 2026-09-21 落地**，change `auth-response-slimming`）

- **server**：`UserInfoVO` 删三字段；`AuthServiceImpl.toUserInfo` 与 `getProfile` 的 map 同步去 `email` / `status` / `guestShortId`，并移除 `buildGuestShortId` 的调用（方法本身若无其它消费方则一并删除）。
- **client**：`types/user.ts` 去三字段；`api/user.ts` 去 `email` 映射与 `deriveCampusEmail` 的邮箱推导（认证表单发码所需推导保留）；`pages/profile/index.vue`、`pages/mine/index.vue` 改读 `bindEmail`；游客短标识改由 `id` 派生。
- **web / 管理端**：`UserVO` 删 `guestShortId`（`status` / `email` 保留）；`types/index.ts` 与 `api/adapter.ts` 同步去声明与映射；`UserView` 的关键词过滤与昵称兜底展示改为按行 `id` 现算「食客 + ID 尾 4 位」（局部函数，复用点 2 < 3 不上提共享层）。
- **文档（本批已改）**：`project_spec.md`（本条 + §5.y.4 / §5.y.5 就地修订）、`api-design.md`（§8 差异登记册）、`feature/client-微信静默登录与游客态.md`（字段表 + 「出参精简落地状态」节；该文档原「答疑」章节已删除）。
- **`feature/client-微信静默登录与游客态.md` 同批补齐的说明性内容**（非契约变更）：JWT 载荷为**明文非密文**、服务端**不存 JWT / 无绑定表**、token 换新的五种时机、签名防伪机制、三条已知边界（多实例黑名单不共享 / 读路径兜底弱于写路径 / 密钥轮换等于全站 token 失效）。

#### 3. 恢复须重新拍板

恢复被删出参（或以任何形式重建「邮箱双源」「服务端出参游客短标识」），均须**重新拍板**（PR-04）。

---

### 7.33 公开契约精简：分页壳去 `list` + 菜品列表参数收敛 + 食堂端点合并（2026-09-21 用户拍板）

#### 1. 口径定型

- **分页壳删 `list` 兼容字段**：`PageResult.list` 与 `records` 恒等值、且参与序列化导致**同一数组被输出两次（列表响应体积≈翻倍）**；两个消费端（client `recordsOf` / web `pageRecords`）均**优先读 `records`** → 该分支永不命中。`getList()` **删除**，分页壳收敛为 `records` / `total` / `page` / `pageSize`。
- **`GET /dishes` 参数收敛**：删 `stallId`（筛选无档口入口、端上零发送）、`sortBy` / `sortOrder`（四值中仅 `heat` 被首页默认流使用，且 `heat` 分支内部固定 `DESC`，`sortOrder` 对任何取值均无效果）；**排序口径收敛为服务端恒热度**——`DishMapper.xml` 去排序 `<choose>`，原 `<otherwise>`（`rating_count DESC, avg_rating DESC`）一并作废，消除「首页传 `heat`、筛选 / 搜索不传参数则落到非热度」的**口径分裂**（合 §7.17 第 2 条「热度优先、不设排序入口」）。
- **食堂端点合并**：`GET /canteens/all` **删除**，合入 `GET /canteens` 的可选参数 `include=stalls`（唯一合法值 `stalls`，非法值按缺省处理）；`CanteenInfoVO` 收敛为 `id` / `name`，`include=stalls` 出参收敛为 `id` / `name` / `stalls[]`（档口项仅 `id` / `name`）；连带删除 `listCanteens()` 的图片绝对 URL 拼接与 `listWithStalls()` 的 `batchAvgRating` 批查（均属出参收敛后的白算）。`api-design.md` §8 已登记的 RESTful 收敛项（`GET /canteens/all` → `GET /canteens?include=stalls`）由「待拍板」转为**采纳**。
- **端上实现层死字段清理**：`MixedResult.ratingCount`（只写不读）、`Dish.image`（与 `images[0]` 同值派生）删除；两处 `recordsOf` / `pageRecords` 的 `list` 兜底移除。

#### 2. 保留项（明确不删）

`total` / `page` / `pageSize` **保留**：分页元数据语义正当（`total` 在评价链路仍为结束判据），仅更正文档定性——首页列表的结束判据是「**本页返回条数 < `pageSize`**」，不是 `total`。公开 `DishVO` 列表 / 详情**共用**（登记为「有意过度下发」，不拆 VO）。

#### 3. 影响面（文档已修订；代码与本条同批落地，change `api-slimming`）

- **server**：`common/result/PageResult`、`dish/dto/DishQueryReq`、`resources/mapper/DishMapper.xml`、`canteen/controller/CanteenController`、`canteen/service/impl/CanteenServiceImpl`、`canteen/dto/CanteenInfoVO` / `CanteenWithStallsVO` / `StallDetailVO`。
- **client**：`api/shared.ts`、`types/dish.ts`、`api/dish.ts`、`api/canteen.ts`、`types/canteen.ts`、`utils/image.ts`、`pages/home/DishCard.vue`、`pages/find/index.vue`、`pages/find/FindResults.vue`。
- **web**：`api/adapter.ts`。
- **文档**：`docs/api-design.md`（§1.4 / §2.1 / §8）、`docs/feature/client-首页菜品浏览.md`、`client-意见反馈.md`、`docs/feature/README.md` 契约表。
- **无库表变更、无数据迁移**；三端同仓同批发布，不保留过渡期。

#### 4. 恢复须重新拍板

恢复 `list` 兼容字段、`stallId` / `sortBy` / `sortOrder` 参数、`GET /canteens/all` 端点或食堂字典扩展出参（`location` / `description` / `images` / 档口层字段），均须**重新拍板**（PR-04 / PR-05）。**注**：`GET /dishes` 的**筛选维度增补**（如后续 change 的菜品大类 `mealType`）不属于本条的恢复项，按其自身决议与 capability 引入。

---

### 7.34 首页 UI 重构：两态结构 + 全站主色改橙 + 菜品大类（2026-09-21 用户拍板）
...
- **大类筛选与字典**：`GET /dishes` 新增单值参数 `mealType`（**白名单校验，非法值 400**，不静默降级），可与 `canteenId` / `keyword` / 价格区间叠加；**新增只读端点 `GET /dishes/meal-types`**（公开）下发 `[{ key, label, order }]`，**只含当前有在售菜品的大类**（空类自动隐藏）。**端上不得写死任何标签文案或标签清单**。分层：公开 `DishVO` **不加** `mealType`（卡片不展示 → 零消费即删）；后台 `DishAdminReq` / `DishAdminVO` 增加（录入下拉 + 编辑回填 + 列表筛选）。
- **不引入「人气推荐」标签**：默认流已为热度序，再加一个热度标签与「全部」重复；若改为第二排序口径则违反 §7.33 的「排序唯一口径 = 热度」。要做运营精选须走「新增 `is_recommended` 字段」而非排序分支。
- **与品类删除决议的边界（重要）**：本次新增的是 `dish` 上的**单值大类字段**，**不是**恢复品类维度——`category` 表、`dish.category_id`、`/admin/categories`、后台品类维护页**均不存在且不得新建**，§7.22 第 1 条「菜品按食堂 / 档口归属，不存在分类维度」**继续有效**。大类目录由后端常量（`MealTypeConst`）定义，**不建字典表、不设外键**；代价是新增大类需改常量并发版（低频变更，用户已接受）；若将来要后台自助增删大类，须另立 change 并先撤销 §7.22 第 1 条。
- **筛选入口形态**：搜索框右侧「筛选」按钮展开既有食堂 / 价格筛选面板（**筛选维度不变**），与大类标签栏**可叠加**；原「搜索框一行 + 筛选按钮一行」的两行头部作废。

#### 2. 影响面（文档已修订；代码随本批落地，change `home-ui-refresh`）

- **server**：`dish` 新增 `meal_type` 列（`schema.sql` 幂等**存储过程** ADD COLUMN，禁直连 ALTER；`seed_data.sql` 按「新增列后回填」惯例幂等 UPDATE 全量赋值，并修正珍珠奶茶 / 杨枝甘露 的 `ingredients` 误标）、`Dish` 实体 / `DishAdminReq` / `DishAdminVO`（+`mealType`）、`DishQueryReq`（+`mealType` 白名单）、`DishMapper.xml`（大类筛选条件 + 在售大类查询）、**新增 `GET /dishes/meal-types`**（Controller / Service / `MealTypeVO`）、大类常量 `MealTypeConst`；`DishVO` **不加**该字段。
- **client**：首页 `pages/home/**`（两态、吸顶、搜索框 + 筛选按钮、大类标签栏、卡片四段、flex 双列）、新增大类字典 api/store、`api/dish.ts`（`mealType` 透传）、`theme/tokens.ts` + `App.vue`（换橙 + 渐变底，双份声明同步）、Banner 插画**本期为占位实现**（首页私有组件 `pages/home/HomeBannerArt.vue`：内联 SVG → `image` data-uri，与 `IconSvg` 同机制）。
- **web**：`variables.css`（主色换橙）、菜品表单大类下拉、菜品列表大类筛选、`api/adapter.ts` 映射。
- **文档**：`docs/feature/client-首页菜品浏览.md`（G / H 决议）、`docs/feature/web-菜品管理.md`、`docs/api-design.md`（`/dishes` 参数 + 新端点 + §8 差异册）。
- **无表结构重建**（仅新增一列，可空）；`meal_type` 为可空列，回滚不影响旧逻辑。

#### 3. 恢复须重新拍板

恢复暖砖红主色、恢复「红色顶栏 / 两行头部」形态、引入「月售」或「人气推荐」标签、把大类改为字典表 / 后台可维护（即恢复品类表形态），均须**重新拍板**（PR-04 / PR-05）。
...
2. ~~**删除 `HotSearchVO.heat` 出参**：`GET /dishes/hot-search` 收敛为仅 `keyword`（端上 `HotSearch` 本就只读 `keyword`，MP-03）；`ORDER BY heat` 保留在 SQL 内（`heatScoreExpr` 计算不变）。~~ **（2026-09-22 用户拍板取代，随 change `search-page-refresh` 落地）**——**端点改名**：`GET /dishes/hot-search` **整体改名 `GET /dishes/for-you`**（旧路径删除、**无兼容别名**）；**语义改随机**：由「按热度取热搜词条」改为**随机抽取在售菜品名**（`ORDER BY RAND() LIMIT #{limit}`，**不再按热度排序，`heatScoreExpr` 在本端点不再参与**）；**去缓存**：响应缓存移除（`@Cacheable(CACHE_DISH_HOT_SEARCH)` 退役）；**类型与端上改名**：`HotSearchVO` → `GuessLikeVO`，端上 `HotSearch` / `getHotSearch()` / `hotSearchList` / `fetchHotSearch` → `GuessLike` / `getGuessLike()` / `guessLikeList` / `fetchGuessLike`，UI 文案「猜你想搜」→「猜你喜欢」。**`heat` 出参删除随本批落地**（端点仅出 `keyword`）。注：`heatScoreExpr` 仍为 `GET /dishes` 默认流的唯一排序口径（§7.33 / §7.34），本条不改动该口径。
3. **动机**：别名层增加后台录入负担且从未成为端上依赖（PR-05 零消费即删）；`heat` 为纯留痕字段。
4. **影响面**：server（mapper / DTO / 实体 / 校验）、web（表单输入 + adapter 映射）、client（~~**无感**——搜索请求不变~~ **2026-09-22 追加：搜索请求确实不变，但热搜端点改名 + 端上标识符 / UI 文案改名落在 client，见第 2 条**）；文档（A-01 / A-03 / web-菜品管理 / web-菜品详情查看 / api-design / database / 本条）。
5. **状态**：**已落地（2026-09-22，change `search-page-refresh`）**——`dish.alias` 全链删除（`schema.sql`「4.7」幂等段 `drop_dish_alias_column`，`CREATE TABLE` 已不再建列；`Dish` / `DishAdminReq` / `DishAdminVO` 字段、`DishMapper.xml` 别名匹配路与列映射、后台表单「搜索别名」输入与「alias ≤255」校验 / 错误码文案全链移除，关键词匹配收敛为菜名 / 档口名 / 食堂名三处）、热搜端点改名 + 语义改随机 + 去缓存同批落地（见第 2 条）；**存量远程库 `dish.alias` 列尚未 DROP**（TDSQL-C 远程连接 1045 拒绝，`schema.sql` 幂等段待用户执行；服务端已不再 select / 匹配该列，对运行无影响）；§8 已留痕。
...
### 7.37 取消小程序端「页面级下拉刷新」（2026-09-22 用户拍板，change `remove-pull-to-refresh`）

1. **决策：整体下线、不留例外**。端上下拉刷新**只有一套实现**——`scroll-view` 的 `refresher-enabled` / `:refresher-triggered` / `@refresherrefresh`，**共 5 处绑定全部删除**：`pages/home/index.vue`（`onRefresh`）、`pages/find/index.vue`（`onResultsRefresh`）、`pages/find/FindResults.vue`（prop `refresherTriggered` + emit `refresh`）、`pages/notifications/index.vue`（`onRefresh`）、`pages/my-reviews/index.vue`（`onRefresh`）。**`FindResults` 对外接口收敛为 `items` / `keyword` / `select`**（不保留「未使用但兼容」的入参，PR-05——留无用 prop 会让规范与代码继续宣称一个不存在的交互）。**`pages.json` 的 `enablePullDownRefresh` 保持不启用**（**从未启用**，端上不存在原生页面级下拉刷新）。`pages/detail/dish/*` 走 `onPageScroll` / `onReachBottom`、无页面级下拉刷新，**不在本批范围**。取舍理由：只留部分页面会造成「同样下拉、有的页有反应有的没有」的不可预期，比全部取消更差；按页配置开关（`FEATURE_GATES` 式）亦否决（零消费即删）。
2. **首页「受控 `scroll-top` 脉冲回顶」连带下线 —— 已知损失（须用户知悉）**：`scrollTopProp` 与 `resetScrollToTop()`（含 `:scroll-top` / `:scroll-with-animation` 绑定）**仅为下拉刷新的「强制回顶」服务**（`scroll-view` 无 `scrollTo`，故用受控 `scroll-top` 脉冲实现），随之下线；首页滚动容器恢复**不受控**（留着即死代码，PR-05）。**首页由此失去唯一的「强制回顶」手势**——**记为已知损失**；如需回顶入口（如浮动「回到顶部」），**须另立 change 单独拍板，本批不夹带**（PR-04）。该「强制回顶」亦与既有口径相左（切分类不回顶、保持吸顶态）。
3. **刷新 / 恢复语义收敛为三条既有路径，不新增任何替代入口**：① `composables/useOnShowRefresh.ts`（**MP-07** onShow 重拉闸门，消费方 = 系统通知页 / 我的评价页）；② 各页**首屏拉取**（`onLoad` / `onShow`）；③ `components/RetryBlock.vue`（**MP-012**「加载失败 · 点击重试」，**5 处消费**：`HomeContent` / `find` / `notifications` / `my-reviews` / `DishReviewSection`）——**重试块升为唯一显式恢复入口**（显式、可发现、不打断浏览位置）。find 结果态恢复路径 = 重试块 或 **重新提交搜索**（顶部搜索行常驻、始终可见）。**明确不做**：无替代刷新手势、无刷新按钮、无「回到顶部」浮动按钮（追加一条交互会重新引入刚被判定为冗余的能力）。
4. **规范真源同步（6 个 capability）**：MP-012 口径由「首屏 / 下拉刷新请求失败」收敛为「**首屏**请求失败 + 重试块」（`client-ui-interaction-a11y` / `client-page-structure` / `my-reviews`）；`find-page-layout` 的「**结果态的下拉刷新行为 SHALL 保留**」**整体推翻并留痕**（恢复路径改为重试块 / 重新提交搜索）；`search-result-presentation` 去掉 `FindResults` 的 `refresherTriggered` 入参与 `refresh` 事件；`contribution-entry` 去掉「下拉刷新过程中不位移」的对照项。
5. **词形相近、语义无关，不在本批范围（不得误删）**：底部 Sheet 的「**下拉关闭手势**」（阈值 ~120px，§4.4 / §4.9）与 Web 菜品表单的「**大类下拉选择**」——验收判据用 `grep -rn "refresher" client/src`（该词只属下拉刷新），勿用宽泛的「下拉」匹配。
6. **影响面**：**仅小程序端**——上列 5 文件 + 注释口径同步（`components/RetryBlock.vue` / `composables/useOnShowRefresh.ts` / `stores/dish.ts`）；**不改**服务端（`server/`）、Web 管理端（`web/`）、数据库（`schema.sql` / `seed_data.sql`）、**接口契约**（本批无接口变更）。文档 = `docs/project_spec.md`（本条 + §5.y.4 就地修订）、`docs/ui/client-首页菜品浏览.md`（§5 边界行为删「下拉刷新：强制重置 `scrollTop = 0`」一条 + 口径行重述）、`openspec/specs/` 下 6 个 capability 主 spec。对应 PR-05（冗余边界：重复刷新路径与死机制清理）与 PR-04（被否决项留痕）。
7. **状态**：**已落地（2026-09-22，change `remove-pull-to-refresh`）**——5 处绑定与首页回顶机制删除，`client/src` 内 `refresher` / `下拉刷新` **零命中**，`npx vue-tsc --noEmit` **0 error**；6 个 capability 主 spec 与 `docs/ui/client-首页菜品浏览.md` 已同步。**恢复须重新拍板**（含恢复首页回顶入口）。
...
### 7.40 菜品详情模块冗余评审（2026-09-23 用户拍板「全采纳」）

> 来源：用户对 `docs/feature/client-菜品详情.md` 发起**冗余设计评审**，产出 7 项、用户**全采纳**。**本轮只更新文档**（`docs/feature/client-菜品详情.md`、`docs/ui/client-菜品详情.md`、本条），**代码待落地**（逐项状态见条目内标注）。
> **2026-09-23 复核补充（用户拍板）**：对 R3 / R4 / R5 / R6 四项给出最终方案 —— **R3 维持 (a)**（与 §7.30 第 8 项既有拍板一致，复核维持）；**R4 由「后端直出中文」修正为「字典下发 + 多值数组化」**（原方案只能解决 client 一端，web 端亦硬编码同一套映射表）；**R5 范围由 1 个 VO 扩至 4 个**；**R6 消费方已核实为零 → 可直接下线**；另立 **PR-12 扩展项**（枚举真源盘点机制，见 §7.20）。**本轮仍只改文档，代码零改动。**
> **2026-09-23 第二轮接口字段评审（用户拍板「落进文档」）**：在本模块冗余评审之后，**再对 `DishDetailVO` / `ReviewVO` / 分页壳的字段设计做一轮专项评审**，产出 **R8～R13**（见下），并**扩展 R1 口径**（原「只改分母」不彻底，卡内数字仍不同源）。**本轮同样只改文档，代码零改动。**
> **✅ 2026-09-23 全部落地（change `dish-detail-contract-hardening`）**：本条 R1～R14 **已由三端 + 库表同批落地**，逐项状态见各条目内的「✅ 已落地」标注。**R14 属「已裁决不改代码」**（维持 `userId`）。库结构变更走 `schema.sql` 幂等段（`migrate_dish_multivalue_json` / `drop_review_updated_at`），**无直连 ALTER**。文档侧差异已清零（见 `docs/feature/client-菜品详情.md` 文末「与当前代码的差异」）。

1. **R1 评分摘要卡「卡内数字不同源」（一致性缺陷，✅ 已落地 2026-09-23；第二轮评审扩展口径）**：`DishSummaryCard` 卡内四个数字来自**两处不同来源** —— 分布条**分子**来自**实时聚合**（`selectRatingDistribution`），而**分母**（`distPct` 取 `props.ratingCount`）、**「N 人评分」文案**、**均分**均取 **`dish.rating_count` / `dish.avg_rating` 缓存列** → 缓存漂移时「各星百分比之和 ≠ 100%」，并与同卡「N 人评分」**自相矛盾**（违反 PR-02 / PR-08）。
   **原口径（仅「分母改用 `sum(count)`」）不彻底**：它只修分母，卡内「N 人评分」与均分**仍是缓存值** → 矛盾会从「分子 / 分母」**搬到「分布条总占比 vs N 人评分」**。**扩展后口径：同一「评分摘要」组件内的全部数字必须同源同刻** —— 分布、总数（「N 人评分」）、均分三者统一取**实时聚合**（一次查询同刻算出），**SHALL NOT** 一部分实时、一部分取缓存列。
   仅当均分因性能考虑确需取缓存时，须在文档**显式登记为有意权衡**并接受与分布的微差（**当前无此登记 → 默认三者全实时**）。
2. **R2 分页壳 `list` 残留登记（已闭环）**：详情功能文档字段表曾登记 `list`（"过渡期兼容字段"），而该字段**已于 2026-09-21 删除**（§7.33）。已从文档清除；`pagination-contract` 的「SHALL NOT 输出 `list`」不变。
3. **R3 「我是否已评价」的判定方式（2026-09-23 拍板：维持 (a)）**：`GET /dishes/{id}` 现无用户态字段（2026-09-16 收口），端上需**单独**调 `GET /my/reviews?dishId=` 且对游客 / 未认证**分支跳过**。三方案：**(a)** 维持现状（结构最干净，代价 = 一次往返 + 一个端上分支）；**(b)** 评价列表响应外挂 `myReview`（**与 `pagination-contract` 冲突** —— 分页壳恒为 `records`/`total`/`page`/`pageSize` 四项，不得挂业务字段）；**(c)** 详情 VO 增加 `myReview`（`ReviewVO | null`，**字段恒在**故字段集登录 / 游客仍一致，代价 = 回退 2026-09-16 的收口）。**结论：(a) 采纳，(b) / (c) 否决** —— (b) 违反分页壳契约；(c) 会让「我的评价」数据在「菜品详情」与「我的评价」**两处下发**（**两套真源、两处改动、两处失效点**），与本次评审「消除冗余真源」的方向**恰好相反**，且使详情接口从「纯公开」变为「半用户态」、失掉缓存 / CDN 友好的架构属性。**依据补充**：`GET /my/reviews?dishId=` 是「我的评价」功能的**合法视图**（按菜过滤），非为详情页造的一次性端点 → **不构成冗余**；详情页三个请求**并行**发出，额外成本 ≈ 1 次轻量 GET。**注：本结论与 §7.30 第 8 项（2026-09-20 已定「我是否已评价」用方案 (a)）一致，本次为复核维持、非新决策；代码零改动。**
4. **R4 四维改「字典下发 + 多值数组化」（2026-09-23 方案修正；契约 + 库表，待排期）**：现状把 4 张「英文机器值 → 中文」映射表**两端各硬编码一份** —— `client/src/api/dish.ts` 与 **`web/src/constants/index.ts`**（**web 那份同时用于展示与表单选项**，经 2026-09-23 核实），连同后端常量表共成**三套真源**（违反 PR-12；且与 `mealType` 走字典下发的既有做法不一致）。
   **原方案（后端直出中文）已修正**：它**只能解决 client 一端** —— web 端表单的 `*_OPTIONS` 仍须硬编码中文选项（下拉 / chips 必须有选项列表），问题被**搬走而非解决**，且会与 `mealType` 的字典模式形成两套并列。**目标形态**：
   - **新增只读字典端点**（模式对齐 §7.34 的 `GET /dishes/meal-types`），下发四维的 `{ field, value, label, order }` 列表；**client 与 web 共用同一份字典**，两端硬编码映射表**一并退役**（client `src/api/dish.ts` 的 `DIET_TYPE_MAP` / `SERVE_TEMP_MAP` / `INGREDIENT_MAP` / `FLAVOR_TAG_MAP`；web `src/constants/index.ts` 的 `*_META` / `*_OPTIONS`）；
   - 业务数据**仍带机器值**（如 `dietType: "meat"`）→ 保留筛选 / 统计锚点，未来若恢复筛选无需改契约；
   - `ingredients` / `flavorTags` **由逗号分隔串改 `string[]`**（存储列 JSON 化**已随本批落地**，见 `schema.sql` 的 `migrate_dish_multivalue_json` 幂等段）→ 消除两端 `split(',')` / `parseCsv` 解析；此属**数据类型规范化**，与字典下发不冲突、**已保留**。
   英文机器值 ↔ 中文的完整对应关系仍以 §7.28 为**真源**（§7.28 第 1 节「映射唯一真源在 client `api/dish.ts`」的口径已被本项推翻，原地标注）。
5. **R5 `imagesJson` 中转字段迁出出参 VO（2026-09-23 范围扩至 4 处；✅ 已落地）**：该字段以 `@JsonIgnore` + `@Schema(hidden = true)` 寄生于出参 VO，仅承接 DB 原始 JSON 串 → VO 被迫承担「DB 行 → VO」的转换职责，且**靠注解兜住不出参**（漏注 / 换序列化器即泄漏）。**范围（2026-09-23 核实）：共 4 个 VO 存在该模式** —— `DishDetailVO` / `DishListItemVO` / `DishAdminVO` / `ReviewVO`（原条目仅登记 `DishDetailVO`，**范围不足**）。**口径**：统一改由 **MyBatis TypeHandler** 直出 `List<String>`（DB 列 ↔ List 的转换下沉到持久层，mapper 的 `resultMap` 同步配 TypeHandler），或 Service 用中间对象承接；出参 VO 只留出参字段。**绝对 URL 转换（`ImageUrlUtil`）仍留 Service 层**，不掺进 TypeHandler（那是业务逻辑）。
6. **R6 `review.updated_at` 零信息（2026-09-23 消费方已确认 → 可直接下线）**：重评时 `created_at` 与 `updated_at` **同批刷新**（见 §7.30 重评口径）→ 两者恒等，该列对评价**无独立语义**，端上零消费。**消费方核实（2026-09-23）：`web/src/views/` 全目录对 `updated_at` 零命中**（仅 `api/adapter.ts` 与 `types/index.ts` 有映射 / 类型声明，**无任何页面渲染**）→ 属**零消费字段**（PR-05）。**口径**：按 PR-07 连同**实体 / DTO / `adapter` / `types` / `schema.sql` / `seed_data.sql`** 成对下线。**⚠️ 落地警示：只删 `review.updated_at` —— `dish.updated_at` 有真实消费（`DishFormDialog.vue` 的 Q-112「他人已修改」轻提示基线），不得误删。**
7. **R7 `userId` 出参 + 端上权限判定 —— 维持（已登记，非冗余）**：端上用 `userId` 决定「删除 / 举报」入口的**显隐**，服务端另有「非本人 → 403」鉴权 → 属**有意的双重判定**（端上需要的是入口显隐、非鉴权本身；用户 ID 非敏感）。**登记为有意的权衡，防止后人误判为冗余而删除**。
8. **R8 异常分支契约（2026-09-23 拍板：专属业务码 `4001` + 下架等价不存在）**：菜品详情侧此前**未登记任何异常分支**，而 UI 稿明确要求「菜品不存在 / 详情拉取失败须给出明确文案与恢复路径（**不得只留纯空白页**）」→ **端上有消费、契约零登记**。**拍板如下**：
   - **专属业务码 `4001`（资源不存在）**：`GET /dishes/{id}` 查不到时返回 **`code = 4001`** + `message = "菜品不存在"`（**取代**现状的统一 `400`）。**⚠️ 本项扩展现有错误码受控集合** —— 原口径为「**仅 200 / 400 / 401 / 403 / 4031 / 500**，资源不存在按 400 业务校验返回」（见 `ReviewServiceImpl` 注释），本次**新增第 7 个码**。理由与 `4031` **同源**：**细分业务码让端上给出差异化引导**（「不存在」需给恢复路径，「参数非法」只需提示；靠 `message` 文本匹配脆弱）。
   - **`4001` 为通用语义「资源不存在」**（非菜品专用）：首期在**菜品详情**落地；其余「查无实体」的读接口（如评价不存在）**随各自变更渐进对齐**，**不强制一次性全改**（现状 `BusinessException(400, "评价不存在")` 保持有效）。
   - **下架菜品 = 不存在**：`status = 'off'` 的菜品，详情请求**与不存在同款处理**（`4001` + 「菜品不存在」）。理由：`status` 已于 §7.27 删除（理由「公开接口恒只返回在售」）→ 「下架」对外**等价于不存在**，契约面最小、**无需新增字段或专用码**。
   - **连带文档**：错误码全集须同步 `docs/feature/README.md`「统一响应信封」表（已补 `4001`）与 `docs/feature/client-菜品详情.md`「异常分支」节。
9. **R9 同名 `ReviewVO` 承载双视角字段集（契约模糊，✅ 已落地）**：`ReviewVO` 在**公开列表**（`GET /dishes/{id}/reviews`）为 **8 字段**，在**本人视角**（`GET /my/reviews`）为 **11 字段**（+ `dishId` / `dishName` / `isHidden`）—— 靠「调用哪个接口」区分，**类型系统无法表达差异**；端上若写单一 interface，则公开场景下那 3 个字段是「类型说有、实际没有」的**不安全类型**。**口径**：拆分为 `ReviewVO`（公开 8 字段）/ `MyReviewVO`（本人 11 字段）；**无论是否拆类，端上 MUST 定义两个类型、SHALL NOT 复用单一 `ReviewVO` 类型**。
10. **R10 端上自造字段名（同一字段两个名，✅ 已落地）**：`ReviewVO.createdAt` 被端上 API 层映射为 **`createTime`**（`client-菜品详情` 与 `client-我的评价` 两处文档**均如此登记**）→ **同一字段两个名**（违反 PR-02 命名一致性 / PR-12 跨端显式定型）。**口径**：端上统一使用 `createdAt`，**删除改名映射**。
11. **R11 服务端口径权威（分页壳归一化规则 + 分布数组顺序，✅ 已落地）**：① **分页壳 `page` / `pageSize` 回传**的价值在于「归一化反馈」，但文档**未写归一化规则** → 补登 **`PageUtil.normalize` 口径**（`page < 1 → 1`；`pageSize < 1 → 10`；`pageSize > 100 → 100`），使回传字段的意义**可自证**（否则读者会误判为冗余）；② **`ratingDistribution` 的返回顺序未约定** → 端上 `useDishPage.ts` 自行 `sort((a, b) => b.star - a.star)` **自排序**，属**口径权威方跑到端上**（违反 PR-02）→ 契约**约定顺序**（建议 **5 → 1**，与展示一致），端上**不再排**。
12. **R12 契约严谨性（可空性 + 实现细节下沉，✅ 已落地）**：① **字段可空性未系统标注**（详情 VO 中仅 `originalPrice` 标了 `| null`），而 `description` / `floor` / `stallName` 等列**实际可空** → 端上无法从契约得知边界。**口径**：逐字段标注可空性，或声明「除标注外均非空」。② **`hasImage` 的筛选条件被写成 SQL 实现细节**（`images IS NOT NULL AND images <> '' AND images <> '[]'`）→ **存储形态泄漏进契约**，且该条件**在 R4 数组化后必然失效**。**口径**：契约只写**语义**「仅返回配图非空的评价」，SQL 条件降为**实现注**并标注「**随 R4 需重写**」。
13. **R13 前置约束登记（字典 `field` 名 + 端上判定例外，✅ 已落地）**：① **R4 字典下发后**，端上 / 管理端需按字典项的 `field` 匹配 VO 字段 → 须约定 **`field` 取值 = VO 字段名**（`dietType` / `ingredients` / `flavorTags` / `serveTemp`），否则前端要维护**第二套映射**；② **`originalPrice > price` 判折扣属「端上算业务口径」**，与 PR-02「同一业务口径的权威方固定为后端」**有张力** —— 但 §7.26 已拍板且判据极简，**维持现状**，本项**显式登记为经拍板的端上判定例外**，防止后人按 PR-02 翻案。
14. **R14 `userId` 保留、不引入 `isMine`（2026-09-23 拍板：维持 `userId`）**：R7 已维持 `userId` 出参；本轮另评估「改下发 `isMine: boolean` 使端上零状态依赖」的替代方案 —— **结论：不采纳**。理由：
   - **⚠️ 决定性 —— `isMine` 会让公开接口变为「半用户态」**：`isMine` 必须由服务端**按请求者**计算，而公开评价列表（`GET /dishes/{id}/reviews`）当前是 **🔓 公开接口、不解析登录态** → 加 `isMine` 即要求它解析**可选 token**。这与同日拍板的 **R3 维持 (a)**（保持公开接口纯粹、不引入用户态）**直接矛盾** —— 刚为详情接口守住的线，会在评价列表上被它破掉。
   - **本人视角恒真**：`GET /my/reviews` 全是本人的评价 → `isMine` 恒为 `true`，属**恒真冗余字段**（该视角不需要它）。
   - **`userId` 的代价可接受**：它是**数据自带字段、不依赖请求者** —— 公开接口无需任何鉴权上下文；端上虽需持有「当前用户 id」比对，但 R7 已登记为**有意的权衡**。
   **防翻案登记**：本条与 R7 一并构成「**评价作者标识维持 `userId`**」的结论 —— 后人 **SHALL NOT** 据「端上零状态依赖更干净」或 PR-02 将其改为 `isMine`；若确要消除端上状态依赖，须**先解决「公开列表是否接受可选登录态」这一架构问题**并**重新拍板**。

### 7.41 浏览计数口径变更：UV → PV（2026-09-23 用户拍板）

> 来源：用户对 `docs/feature/client-浏览计数.md` 提出「**不做人员与时间限制，load 一次即 +1**」的主张，经评估后**采纳**（含三项配套决策）。
> **✅ 已落地（2026-09-23，change `view-count-pv`）** —— 代码改动已完成并实测通过（差异清单已清零，见该文档文末「与当前代码的差异」）。

**结论**：浏览量口径由 **UV（同一用户对同一菜品每自然日只计 1 次）** 改为 **PV（每次进入详情页即 +1）**。§7.14 第 1 条（原拍板的当日去重）**整体作废**，仅作历史留痕。

1. **不做人员限制 —— 含游客**：`POST /dishes/{id}/views` **转为公开端点**（`permitAll`）。
   缘由：详情页是 🔓 公开页，原「需登录」使**游客浏览一次都不计**，与实际流量结构严重不符。
   **连带风险已评估并接受**：匿名写接口可被匿名刷 —— 由下述第 3 条兜底。
2. **不做时间限制 —— 删除两道去重**：
   - **5 分钟内存窗口**（`dish/config/ViewRateLimiter`，key=`userId:dishId`）：**整体退役**。理由：与「当日去重」**功能重叠**（后者已完全防住重复计数），且为**进程内内存实现**（多实例不一致、重启清零）→ 属**冗余防护**；去重取消后该类**失去唯一用途**。
   - **当日去重**（`HistoryService.existsTodayDishView`，判据 `view_log.updated_at >= 今日零点 Asia/Shanghai`）：**删除该判据**。
3. **防护兜底 —— 保留 IP 维度限频**：复用既有 `common/config/IpRateLimiter`（滑动窗口、进程内实现），**本次唯一的限制 = 同一 IP 的请求速率**。
   不限制「谁」「何时」，对正常用户**无感**（正常浏览速率远低于阈值），但可挡住脚本级滥用。
   **（有意的权衡 —— 防止后人以「既然不做限制」为由把这一层也删掉，导致匿名写接口裸奔。）**
4. **`view_log` 保留为浏览足迹**：去重取消后它**不再参与计数逻辑**，仅作为浏览足迹保留（供管理端「学生行为查看」等，见 §8 待办 #8）；upsert 语义与「游客不写足迹」**不变**。**不删表、不删列。**
5. **热度权重影响（本决策的依据之一）**：`heatScoreExpr = view_count * 1 + rating_count * 100 + avg_rating * 20` —— 浏览量权重为 **1**（10 条评价 = 1000 分）→ **刷浏览对排序的影响有限**，故取消去重后排序抗刷能力的下降幅度**可控**。
6. **语义变更须知**：`view_count` 由「独立访客数」变为「**页面浏览量（PV）**」—— **字段名不变、语义已变**；对外若展示该数字，应按 PV 理解。

**连带文档（已同批更新）**：`docs/feature/client-浏览计数.md`（正文重写 + 差异节）、`docs/ui/client-浏览计数.md`（埋点口径）、§7.14 第 1 条（作废留痕）、§5.z **D-B**（`view_log` 用途收窄）、§0.3 与 §7.25 的 `view_log` 表述（同步）、`docs/feature/README.md` 待办 #3（由「建议不改」改写为「已拍板采纳」）。

**取舍登记（如实记录张力）**：本条要求的「**不做限制**」与原 §7.14 第 1 条的「**需防刷、保证热度真实**」是**方向相反的两条**。用户已明确选择前者并接受由此带来的排序可刷性上升（依据第 5 点：浏览量权重低、影响可控）。**恢复去重须重新拍板**（PR-04）。

### 7.42 首页 Banner 能力登记 + 表基线更新（2026-09-23 补登，QA 门禁 P1-6）

> 来源：QA 质量门禁（2026-09-23）发现 —— **`GET /banners` 与 `banner` 表能力已上线，但权威 spec 未登记**，
> 且 §5.x 状态枚举条仍写「Banner 已随公告 / 活动下线移除」（**过时**）。本条补齐登记，**Banner 口径以本条为准**。

1. **能力现状（已落地 2026-09-22，change `home-ui-refresh`）**：首页顶部 **16:10 多图轮播**。
   - **端点**：`GET /banners`（🔓 公开）→ 出参**仅 2 字段** `id` / `imageUrl`（绝对 URL）；**只返回启用项**（`status='on'`），按 `sort_order` 升序；`sort_order` / `status` **不出参**；空集返回 `[]`。
   - **无跳转能力**：`targetType` / `targetId` / `targetUrl` **不出参** —— 要支持跳转须先由 UI 文档定义交互、再扩字段（本节不预埋）。
   - **端上无独立页面**：由 `pages/home/index.vue` 首块渲染；**本期无管理端录入入口**，素材由 `seed_data.sql` 维护。
2. **数据表基线 10 → 11 张**：新增 `banner` 表（`schema.sql` 幂等建表 + `seed_data.sql` 灌初始素材）。**自本条起表基线为 11 张**（此后各条涉及表数的表述以 11 为基准）。
3. **与 §0.0「轻运营 / 无运营位」的冲突登记（⚠️ 待拍板）**：Banner 属**运营位**，与本项目「轻运营」定位存在张力。
   **现状：已上线且经用户拍板（2026-09-22）** —— 故此处**如实登记冲突、不擅自裁定其去留**；
   如需重新评估「是否保留运营位」，**须重新拍板**（PR-04）。相关决议见 `docs/feature/client-首页菜品浏览.md` §I 组。
4. **连带修正**：§5.x「状态枚举」条中「Banner 已随公告 / 活动下线移除」**就地标注作废**（见该条修订注）；
   §5.x「字段命名」条中「原 Banner / 广播契约」表述**保留** —— 该处仅借历史字段名说明通用跳转命名规范，与 Banner 是否在册无关。
...
- **已修复（本轮）**：
  ...
  - **星级组件统一**——评价星级展示统一走同一星级组件（评分口径见 §7.14；~~「有用」置顶口径~~ 已随该能力于 2026-09-20 全链下线，本条不再引用）。
...
- **菜品价格字段精简——删除「折扣价」`promo_price`（2026-09-18 用户拍板，见 §7.26；文档层已修订，代码已于 2026-09-20 落地）**：`price`（现价，已含折扣）+ `original_price`（原价，可空）两态表达折扣，判据 `original_price > price`；`schema.sql` 幂等段先 `UPDATE dish SET price = promo_price WHERE promo_price IS NOT NULL` 再 DROP 列，`seed_data.sql` 删除 `promo_price` 赋值；影响 server / client / web 三端与 `api-design.md` / `database.md` / ~~`product-blueprint.md`~~（已于 2026-09-20 删除）/ 4 份 feature 文档。对应 PR-05（冗余边界）与 PR-07（字段生命周期成对处置）。
- **公开菜品 `DishVO` 出参精简（2026-09-18 用户拍板，见 §7.27；文档层已修订，代码已于 2026-09-20 落地）**：删出参 `status`（公开接口恒只返回在售、服务端已过滤）/ `createdAt`（端上只用 `updatedAt`）/ `canteenId`（端上用食堂名 + 字典）/ `stallId`（只写不读）/ `viewCount`（**只为热度服务，不出参；口径定论 = 一直累计、不清零**）＋ §7.26 的 `promoPrice`，公开 VO **24 → 18 字段**；`DishAdminVO` 的 `status` / `stallId` / `createdAt` 保留、`viewCount` 同步删（Web 无展示消费）；**页面 UI 本批不动**。对应 PR-05（冗余边界）。
- **菜品描述维度替换（2026-09-20 用户拍板，见 §7.28；文档层已修订，代码已于 2026-09-20 落地）**：删 `spice_level`（辣度）/ `region`（风味 / 菜系），新增 `diet_type`（荤 / 半荤 / 素 / 清真）/ `ingredients`（主料）/ `flavor_tags`（口味，吸收辣度）/ `serve_temp`（冷热）；**首页「辣度」筛选与 `DishQueryReq.spiceLevel` 一并删除**；公开 `DishVO` → 20 字段；`schema.sql` 幂等迁移（先加新列、后 DROP 旧列），`region='清真'` → `diet_type='halal'` 的存量转换由用户执行；影响 server / client / web 三端与 `api-design.md` / `database.md` / ~~`product-blueprint.md`~~（已于 2026-09-20 删除）/ 5 份 feature 文档。对应 PR-05（冗余边界）与 PR-07（字段生命周期成对处置）。
- **删除产品蓝图文档 + 删除菜品标签 `tags`（2026-09-20 用户拍板，见 §7.29；文档层已修订，代码已于 2026-09-20 落地）**：① `docs/product-blueprint.md`（《产品定型总纲 v1.0》）**删除**，功能口径统一参照 `docs/feature/`；② `dish.tags`（必吃推荐 / 招牌菜）全链删除——列 / 三端 VO·DTO·映射 / `DishQueryReq.tag` / `TagLabel` 公共组件 / Web `TAG_OPTIONS`；公开 `DishVO` 20 → **19 字段**；`schema.sql` 幂等 DROP `dish.tags`。对应 PR-05（冗余边界）与 PR-07（字段生命周期成对处置）。
- **接口 RESTful 化 + 菜品详情模块优化（2026-09-20 用户拍板，见 §7.30；文档层已修订，代码已于 2026-09-20 落地）**：① **RESTful 原则写入 §3**（资源导向 / 方法表达动作 / 子资源嵌套 / 禁查询参数表归属 / 禁路径动词）；**本批改「评价 + 浏览」两域**——`GET /dishes/{id}/reviews`、`POST /dishes/{id}/reviews`、`POST /dishes/{id}/views`、`GET /my/reviews?dishId=` 加过滤；管理端 / 登录域 / 热搜 / 上传等**登记待拍板**。② **详情页「我是否已评价」定死方案 (a)**：额外调 `GET /my/reviews?dishId=` 判定并取回我的评价 id（修分页边界 bug）。③ **零消费字段删除**：公开评价列表不再返回 `dishId` / `dishName` / `isHidden`；公开 `DishVO` 删 `windowNo` / `updatedAt`（**20 → 17 字段**）。④ **UI 收敛**：信息卡指标条去「评分 / 评价」两列（归评分区）、去掉「信息更新于 X」。对应 PR-05（冗余边界）与 PR-07（字段生命周期成对处置）。
- **坐标与距离概念全链下线（2026-09-20 用户拍板，见 §7.31；文档层已修订，代码已于 2026-09-20 落地）**：`canteen.latitude` / `canteen.longitude` 两列幂等 DROP（并删除 `add_canteen_location` 迁移段与 `seed_data.sql` 两列赋值）、`DishVO` / `CanteenInfoVO` 删坐标出参（`DishMapper.xml` 去 `COALESCE` 兜底）；端上删 `distance` / `utils/location.ts` / `CAMPUS_CENTER` / `formatDistance` / `location` store / `geo-prompt` 与首页·搜索·详情三处距离展示；`manifest.json` 撤 `scope.userLocation` 与 `requiredPrivateInfos` 的 `getLocation`；**公开 `DishVO` 17 → 15 字段**。位置表达收敛为「食堂 · 楼层 · 档口名」。对应 PR-05（冗余边界）与 PR-07（字段生命周期成对处置）。
- **登录出参精简（2026-09-21 用户拍板，见 §7.32；文档层已修订，代码已于 2026-09-21 落地——change `auth-response-slimming`）**：`UserInfoVO`（`POST /auth/wechat-login` 与 `GET /auth/profile` 共用）删 `email`（微信体系下无写入点、**恒为 NULL** 的死字段）、`guestShortId`（后端实现即「食客 + `id` 尾 4 位」，**纯派生**）、`status`（端上零消费，禁用 / 注销由登录侧 400 与 UGC 写操作侧 403 拦截）；校园邮箱唯一出参来源收敛为 `bindEmail`（端上 `bindEmail || email` 双源兜底与 `deriveCampusEmail` 推导一并去掉）；游客短标识改由消费端按 `id` 派生（**小程序与管理端一视同仁**，无 `id` 时回退本地游客 ID）；**管理端 `UserVO` 同批删 `guestShortId`**（`status` / `email` 保留）。**契约回归**：`SmokeApiTest` 新增 2 条断言（登录链路 `userInfo` 键集合精确等于 6 字段、资料链路字段集与登录链路完全相同，并断言 `email` / `status` / `guestShortId` 均 `doesNotExist`），`23/23` 通过，契约层纳入 CI。同批在该域功能文档补齐说明性内容：JWT 载荷为明文非密文、服务端不存 JWT 且无绑定表、token 换新的五种时机、签名防伪机制、三条已知边界（多实例黑名单不共享 / 读路径兜底弱于写路径 / 密钥轮换等于全站 token 失效）。对应 PR-02（口径单一真源）与 PR-05（冗余边界）。
- **术语专项收口 + 设计资产零消费清理（2026-09-21，`dish-detail-remediation` 归档前置处置；**执行既有决议、无新增决策**）**：① **术语正名全文归零**——按 §7.23 第 6 条既定口径（统一「微信内容安全检测（`msgSecCheck`/`imgSecCheck`）」），把 `server/src`（main 33 处 + 测试若干）、`web/src`（7 处）残留的「机检 / 机审」共 **53 处 / 23 文件**统一替换（均为注释 / 文档字符串，无逻辑与常量变更）；`docs/` 侧遗漏的 3 处裸旧词（`feature/client-写评价.md`、`client-个人资料.md`、`web-评价管理.md`）一并正名；**仅保留定义该映射的留痕**（「旧称『机检』废止」类表述）。② **设计资产零消费清理**——按 PR-05「零消费即删」删除 `COLOR_MAP.like` 与 `--color-like` 变量映射（`theme/generated-colors.css` 由 `gen:tokens` 重生成）及零引用图标键 `thumb`（有用/点赞）/ `clock` / `fire`，并同步更新 §4.2 / §4.9 / §3 的图标与色值语义登记、`IconSvg.vue` 用法示例与零消费注释、`dead-export-scan.md` 登记（**恢复须重新拍板，PR-04**）。③ **文档对账**——`dish-detail-remediation` 各决议（§7.26–§7.31）的「影响面（文档层已修订，代码待落地）」与 §8 留痕条目、`feature/client-菜品详情.md` 已拍板清单 #5 / #9 的「待三端 + 库表代码落地」共 15 处过期标注改为**已落地**；`feature/client-评价有用.md` 加「已全链下线」横幅、`feature/README.md` A-07 行同步（该文档仅作历史留痕）。④ **门禁复核**：`client npm run type-check` EXIT 0、`web npm run build` EXIT 0、`server mvn -q test` EXIT 0（`SmokeApiTest` 实跑，覆盖 4031 / 内容安全检测 400 拒绝 / 资源不存在 400 无 404）。**仍待用户执行**：真机 + 真库端到端冒烟、存量库幂等迁移段（`schema.sql` 4.1–4.5 与 `migrate_region_to_diet_type.sql`）。对应 PR-05（冗余边界）与 PR-12（枚举 / 语义展示与后端常量同源）。
- **菜品详情模块审计整改批（2026-09-20 审计产出，用户拍板「P0 先修、其余按推荐处理」；文档已改，代码已落地）**：① **P0-1 价格展示双源修复**（`hasPromo` 改判 `originalPrice > price`、展示值恒取 `price`，见 §7.26 补注）；② **P0-3 分页结束判据改用 `total`**（见 §7.30 第 5 条）；③ **P0-2 已评价态入口**（底栏「写评价 / 重新评价」双态 + 提交成功后就地保持，见 §7.30 第 1 条补注）；④ 一致性清理——死代码（零消费 `loading` prop / 死 emit / 空壳 `onShow` / 只写不读 `sharedDish.stallId`）、术语正名（「机检」→ 微信内容安全检测）、token 收敛（硬编码圆角 / 冗余 fallback）、触控目标 ≥88rpx 与 `aria-expanded`、详情页大图关自动轮播、页面失败 / 不存在态文案（加载态遵守既有「不设骨架」红线）、四维逐维渲染不占位。影响面：`client/src/pages/detail/dish/**`、`api/review.ts`、`types/review.ts`，**无库表变更**。对应 PR-05（冗余边界）与 §4 UI 红线。
...
- **搜索契约精简批（2026-09-21 用户拍板 + 2026-09-22 追加拍板，见 §7.35；文档已修订，代码已于 2026-09-22 落地——change `search-page-refresh`）**：① **删除 `dish.alias`**（列 / `Dish` 实体 / `DishAdminReq` / `DishAdminVO` / 后台表单输入 / `DishMapper` 匹配路与列映射全链）——关键词直接硬匹配菜名 / 档口名 / 食堂名三处（**存量远程库 `dish.alias` 列仍待用户执行 `schema.sql` 幂等段 `drop_dish_alias_column`**——远程连接 1045 拒绝所致，服务端已不再 select / 匹配该列，对运行无影响）；② **~~`GET /dishes/hot-search` 收敛为仅 `keyword`、`ORDER BY heat` 保留在 SQL 内~~ 已由 2026-09-22 拍板取代**：端点**整体改名 `GET /dishes/for-you`**（旧路径删除、无兼容别名）、语义改为**随机抽取在售菜品名**（`ORDER BY RAND() LIMIT #{limit}`，不再按热度排序）、**响应缓存移除**，`HotSearchVO` → `GuessLikeVO`，端上 `HotSearch` / `getHotSearch()` / `hotSearchList` / `fetchHotSearch` → `GuessLike` / `getGuessLike()` / `guessLikeList` / `fetchGuessLike`、UI 文案「猜你想搜」→「猜你喜欢」（**`heat` 出参删除随之落地**）；③ **服务端进程内读缓存整包退役**——`CacheConfig`（含 `@EnableCaching`）、`pom.xml` 的 `caffeine` 依赖、`DishServiceImpl` 的 `@Cacheable` / `@CacheEvict` 残留与读缓存失效时序注释全量删除，`CACHE_DISH_HOT_SEARCH` 常量退役；**结论：服务端当前无任何进程内读缓存**（§5.z D-D 中「Caffeine 短 TTL 缓存(60s) + 写失效现仅剩热搜词条」一句随之不再成立，见该条修订注；恢复读缓存须重新论证并登记）。动机：别名层增加后台录入负担且端上从未依赖（`HotSearch` 端上只读 `keyword`，MP-03）、随机语义与响应缓存相冲突；对应 PR-05（零消费即删）。**状态：已落地（2026-09-22，change `search-page-refresh`）**。
