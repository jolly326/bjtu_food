...
> **取消人工复核见 §7.24（2026-09-15 用户拍板）**：机检 `pass` / `review` 一律放行、仅 `risky` 拒绝；内容安全态 `sec_state` 列与全链能力**已退役**，管理端不设内容复核队列（评价管理 + 反馈处理双页、一级导航 4 项）。**与 §7.24 冲突的历史表述已就地修订并标注来源**。
...
4. **合规底线**：全部 UGC 过微信内容安检（`msgSecCheck` / `imgSecCheck`；**2026-09-15 用户拍板「取消人工复核」，见 §7.24**）——机检 `pass` 与 `review` **一律放行**、`risky`（含未知 / 缺失态 fail-closed 同按 risky）**拦截**；评价对他端可见**不设安检闸门**（唯一判据 `is_hidden=0`）。
...
3. **运营（后勤）**：**反馈处理是唯一的运营闭环**（2026-09-15 蓝图 v1，见 §7.23 第 5 条）——后台处理反馈（含据此录入 / 修改 / 下架菜品）→ 已认证提交人收站内回执 → 小程序即时体现。UGC 文本与配图先过微信内容安检（见 §5.a）；**无人工复核**（2026-09-15 用户拍板，见 §7.24）——机检 `pass` / `review` 一律放行、`risky` 拦截，管理端仅做事后处置（评价隐藏 / 删除）。
...
3. **用户 UGC —— 评价类**：**菜品评价是唯一的「评价」形态**（社区/动态下线后不再有第二种评价语义载体）。菜品详情页评价区（展示 + 内联加载）+ 底栏「写评价」提交；一人一菜一评（`uk_review_user_dish`），评价侧「有用」点赞一人一票（`uk_useful_user_review`）。**评价支持配图（≤3 张）**，文本与图片均须过微信内容安检（机检 `pass` / `review` 一律放行、`risky` 拦截，**无人工复核**，见 §5.a 与 §7.24）。
...
- **明确不做（维度 / 字段 / 实体语义三类，2026-09-14 登记，来源与日期如下，恢复须重新拍板）**：
  ...
  - **菜品独立审核（2026-09-15 蓝图 v1，见 §7.23 第 4 条；2026-09-15 阶段4 用户批准全量退役）**：**`dish.audit_status` 列与索引已删除**（不再作为运营处理入口）；`dish.reject_reason` 退役为历史列（列保留、恒 NULL，不写入）；两项均**不再作为运营处理入口**（后台无审核列 / 无审核模块、客户端无「菜品审核」概念）；学生的菜品诉求走反馈闭环。恢复须重新拍板。
...
- **内容安全安检（2026-09-13 立；2026-09-15「取消人工复核」修订，见 §7.24）**：全部 UGC（评价 / 反馈的文本与配图）须经微信内容安检（文本 `msgSecCheck` v2 / 图片 `imgSecCheck`）；安检**违规一律以 `400` 返回**（文本 `suggest=risky`、未知 / 缺失态 fail-closed 同按 risky；图片微信 code `87014`），**不新增错误码**；**机检 `review`（疑似）与 `pass` 一律放行，无人工复核、无安检态落库**（`sec_state` 列已全链退役；契约细则见 §5.a 与 `docs/api-design.md` §4）。
...
- **文本安检**：`msgSecCheck` v2（`openid` + `scene` + `version=2`）；`scene` 映射：**昵称=1、评价/反馈=2**；**判定口径（2026-09-15 用户拍板「取消人工复核」）= 二态**——`pass` **与 `review`（机检疑似）均放行**（`SecSuggest.fromValue` 在判定入口把 `review` 归一为放行态）、`risky` 拦截（HTTP `400`）；**未知 / 缺失态 fail-closed 同按 risky 拦截**（宁可误拦、不放行）。判定必须以 `result.suggest` 为准，不得只看 `errcode`。
- **图片安检**：`imgSecCheck`；违规（微信 code `87014`）拦截（HTTP `400`）。
- **无安检态落库（2026-09-15 全链退役，见 §7.24）**：`review.sec_state` / `user_feedback.sec_state` 两列**已删除**（存量库由 `schema.sql` 末尾幂等段 `drop_sec_state_columns` 清理，可重跑）；`SecStateConst`、实体 / DTO / VO 的 `secState` 字段、Mapper 过滤条件（含评分聚合的 `sec_state='pass'`）、`PUT /admin/reviews/{id}/sec-state` 端点、`OperationLogConst.ACTION_REVIEW_SEC_STATE`、评价 / 反馈列表的 `secState` 筛选入参**一并删除**。**机检结论不再落库，也不构成任何可见性闸门。**
- **可见性规则**：评价公开展示条件 = `is_hidden=0`（**唯一判据**，后端过滤、前端不兜底）；机检只作**提交闸门**（risky 拦截、其余放行），不参与事后可见性，故前端不再有「审核中」提示（`isHidden` = 「已被隐藏」保留）。
- **反馈安检处置口径**：反馈**无公开展示**——机检违规（`risky` / `87014`）→ `400` 拦截、**不落库**；机检 `pass` / `review` 一律正常落库，**不留复核标记**。反馈事后处置走既有处理闭环 `PUT /admin/feedbacks/{id}`（`is_hidden` 隐藏 / 删除仅评价适用）。
...
- **机制澄清（2026-09-14 核实；2026-09-15「取消人工复核」修订）**：评价侧**无 `audit_status` 字段、无人工前置审核态、亦无人工复核态**。`review` 表以 `is_hidden`（默认 `0`）为唯一可见性开关（`sec_state` 列已退役）。「先发后审」的实际机制为：机检 `pass` / `review` 一律 → 立即对外可见（`is_hidden=0`）；机检 `risky` → 提交时即被 `checkText` 抛 400 拦截、不落库。管理员事后通过 `PUT /admin/reviews/{id}/hide`、`DELETE /admin/reviews/{id}` 处置。
...
- 机检失败 / 判定缺失为 **fail-closed**（`ContentSecurityServiceImpl`：微信调用失败抛 500 拦截；`suggest` 未知 / 缺失态按 risky 拦 400——两条路径均不放行）；图片 >1MB 直接拒绝上传。
...
1. UGC 准入双约束**仅作用于评价（含评价配图）**：写评价 / 评价传图须 `verified=1 && openid != null`。**准入失败按原因分码（2026-09-14 裁定修订，以实现为准）**：`verified != 1` → **`4031`**（`@RequireVerified`/`RequireVerifiedAspect` 触发，message「请先完成学号邮箱认证」，端上弹认证引导 `AuthSheet`）；`verified=1` 但 `openid` 为空 → **`403`**（message「请使用微信登录后再发布评价」，端上提示微信登录，**不弹邮箱认证引导以免误导**，见 `ReviewServiceImpl`）。两种情形的用户引导动作不同，故有意分码；**原「统一返回 403」表述作废（2026-09-14 用户确认，见 §7.21 第 5 条）**。
   **反馈（含投稿 `type=add`、纠错 `type=error`、举报 `type=report`）不设认证门槛**，保持公开免认证 —— 否则会把 §7.3 的学生投稿通道掐死（新用户未认证即无法投稿，与该条决议自相矛盾）。
   反馈虽免认证，服务端机检照常执行：有 openid 时正常调 `msgSecCheck v2`（`risky` 拦 `400`，`pass` / `review` 放行）；无 openid 时按既有边界**跳过机审放行**（`msgSecCheck v2` 必填 openid、无法调用）。反馈内容**不对外公开展示**（仅管理员在后台可见），故该边界暴露面可接受。**2026-09-15 取消人工复核后不再有「落标记进复核队列」动作**（`sec_state` 已全链退役，见 §7.24）。
...
3. **点赞准入**（**2026-09-15 蓝图 v1 修订，见 §7.23 第 2 条**）：评价「有用」（`POST /reviews/{id}/useful`）**需学号邮箱认证（`verified=true`）**——未认证返回 **`4031`**，端上弹 `AuthSheet` 认证引导（入口不置灰）；**不适用 §7.5 双约束中的 openid 约束**（点赞不产生公开可见内容、不触发内容机检，故不要求 openid）。§7.5 的双约束**仅约束「产生公开可见内容并需机检」的评价正文与配图（`POST /reviews`）**。
...
4. **菜品无独立审核**。管理员录入 / 编辑即生效（**公开可见性唯一判据 = `dish.status='on'`**）；**学生的菜品诉求以反馈形式存在并由反馈处理闭环，客户端与后台均不出现「菜品审核」概念**。
   ...
   - `dish.reject_reason` 自此为**退役历史列**：列保留、不再扩展、业务代码不写入（恒 NULL）；「不采纳 / 退回」语义由 `user_feedback.reject_reason` 唯一承载。
5. **反馈处理是唯一的运营闭环**：
