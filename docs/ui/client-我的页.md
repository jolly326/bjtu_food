...
- 页面构成（自上而下，**四块 + TabBar**）：固定标题带「我的」→ **用户卡** → **功能宫格（2×2）** → **「其他」分组列表** → 版本行 → `TabBar`。
...
- 形态：**两行两列**等宽等高白卡（`--radius-card` + `--shadow-card`），格间距 `--spacing-md`，**每格整格热区**。
- 顺序固定：
...
| 4 | 我的浏览菜品记录 | `clock` | **功能未上线**：点击仅 Toast「暂未实现」，不跳转 | — |
...
| 行 | 文字色 | 点击行为 |
|---|---|---|
| 用户协议 | `--text-body` | → `pages/privacy/index` |
...
| 认证态判据 | `bindEmail` 非空（单点收敛） | 决定用户卡两态与「去认证」显隐 |
