...
| PUT | `/reviews/{id}` | **🔐 认证**（作者本人） | **重新评价（覆盖式）**：覆盖同一行的评分 / 文字 / 配图，时间取当前、排到最前 |
...
`DishDetailVO`（**详情专用**，15 字段 + `ratingDistribution` 评分分布）。

> **2026-09-22 列表 / 详情出参拆分**（见 [client-首页菜品浏览](./client-首页菜品浏览.md) D 项）：`GET /dishes` 列表改返**列表专用 8 字段** `DishListItemVO`（`id` / `name` / `coverImage` / `price` / `originalPrice` / `avgRating` / `canteenName` / `stallName`），**不再复用本 VO**；下表 15 字段即**详情专属字段集**，由 `GET /dishes/{id}` 独占下发。

`DishDetailVO` 单行字段（15 个；2026-09-20 §7.31 起删 `latitude` / `longitude`）：
...
| `hasImage` | number | 否 | **「只看有图」筛选**：`1`=只看有图 / `0` 或缺省=不限；筛选条件 `images IS NOT NULL AND images <> '' AND images <> '[]'`。**端上切换筛选时必须重置 `page=1` 并清空列表** |
| ~~`sort`~~ | — | — | **已废除**：排序唯一为时间倒序，端上与后端均不再持有排序切换 |
...
| ~~`review_useful`~~ | **随「有用」下线整表删除** | 有用投票记录与计数 |
| `view_log` | 写（A-05） | 浏览足迹与当日去重 |

## 与当前代码的差异

**无（文档与代码一致）**——截至 2026-09-22，本清单全部条目已落地（正文即最终形态）。

> 本节按 2026-09-22 定稿规则设立：**正文只写最终设计形态，与现有代码的差异一律写在本节**（含「已拍板未落地」条目）；差异清零时保留标题并写「无」。
> 下表为**已落地的历史清单（留痕）**，不再作为待办使用：
