...
> **可空性（R12）**：本字段**恒在**（不为 `null`）；无任何评价时为 5 条 `count: 0` → 端上按「合计为 0」决定不渲染评分卡（**不靠字段缺失判断**）。

> **「我是否已评价」不在本 VO 出参中** —— 由端上单独调「我的评价（按菜过滤）」接口判定，故游客与登录的 `GET /dishes/{id}` 返回结构一字不差；`updatedAt` 不出参（见 `project_spec.md` §7.30）。
> 价格展示只消费 `price`（现价）与 `originalPrice`（原价），折扣由 `originalPrice > price` 表达，**无独立「折扣价」字段**。
> **口径归属登记（R13）**：`originalPrice > price` 是**端上算的业务判定**（PR-02「同一业务口径的权威方固定为后端」的例外口径）—— 折扣以两态表达，判据极简且无歧义。
...
> **公开列表出参 = `ReviewVO` 8 字段**：不含 `dishId`（恒等于路径）/ `dishName`（联表冗余）/ `isHidden`（公开列表恒未隐藏）——三者仅「我的评价」视角需要。
...
> **双视角命名（R9）**：本表为 **`ReviewVO`（公开视角，8 字段）**。`GET /my/reviews` 返回的是**本人视角、11 字段**（本表 8 字段 + `dishId` / `dishName` / `isHidden`）—— **两视角 MUST 是两个类型**（`ReviewVO` / `MyReviewVO`），端上 **SHALL NOT** 用单一 interface 复用。
...
| 场景 | 响应 | 端上处置 |
|---|---|---|
| `id` 不存在（含已被物理删除） | **`code = 4001`** + `message = "菜品不存在"` | 展示「菜品不存在」文案 + **返回首页**的恢复路径 |
...
| `review` | 读（过滤 `is_hidden=0`）；「重新评价」会 UPDATE 同一行（`created_at` 同批刷新） | 可见评价列表。**`review` 表无 `updated_at` 列**（重评时 `created_at` 刷新为新时间，端上与管理端对该列零消费）；**`dish.updated_at` 保留**（列表排序 + 「他人已修改」轻提示） |
| `view_log` | 写 | 浏览足迹。浏览量为 **PV 口径（不去重，§7.41）**，`view_log` 仅作足迹保留 |

## 设计决议（R3 / R14）

**R3（「我是否已评价」的判定方式）：端上单独调 `GET /my/reviews?dishId=`。**

- 评价列表响应**不**外挂 `myReview` → 与 `pagination-contract` 一致（分页壳恒为 `records`/`total`/`page`/`pageSize` 四项，不挂业务字段）；
- 详情 VO **不含**用户态字段 → 「我的评价」数据只在**我的评价**一处下发（单一真源）；详情接口保持「纯公开」，缓存 / CDN 友好；
- **代价登记**：对已认证用户多一次轻量 GET；端上多一个「游客 / 未认证 → 跳过该请求」的分支。**（有意的权衡。）**
