...
用**学号邮箱**（`{学号}@bjtu.edu.cn`）做身份认证，认证通过后 `verified=true`，解锁 UGC 写操作（写评价、评价点赞）。仅限校内邮箱域名，**无密码、无注册**。流程：在 `AuthSheet` 里填**学号**（端上自动推导校园邮箱）→ 发码 → 输入验证码核验 → 成功后返回新 token + 账号信息（`verified=true`、`bindEmail` 已写入），弹层关闭、UGC 入口解锁。

## UI

> 📐 页面 UI 设计稿已拆出 → [client-邮箱认证.md（docs/ui）](../ui/client-邮箱认证.md)（**UI 口径以该文件为唯一真源**）
