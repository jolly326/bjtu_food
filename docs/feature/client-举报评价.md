...
举报违规评价，交给管理员处置（隐藏或删除）。**举报与意见反馈共用同一条提交接口与同一张表、同一个后台处理页**——举报是 `type=report` 的一种反馈。

## UI

> 📐 页面 UI 设计稿已拆出 → [client-举报评价.md（docs/ui）](../ui/client-举报评价.md)（2026-09-22 拆分；**UI 口径以该文件为唯一真源**）
...
2. 填举报理由（必填）→ 提交。
3. 提交成功 → 弹窗关闭、提示处理预期。
4. 管理员在 Web「反馈」页处理 → 若举报人是已认证用户，会收到站内回执（见 A-12）。
...
| 字段名 | 类型 | 必填 | 中文解释 |
|---|---|---|---|
| `type` | string | **是** | 反馈类型，**举报固定传 `report`**（可写值域：`suggestion`/`add`/`error`/`report`；`bug`/`other` 为历史遗留、禁止新增） |
