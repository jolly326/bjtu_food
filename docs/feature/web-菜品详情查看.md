...
看一道菜的**完整信息 + 全部评价（含被隐藏的）**，并在此页直接编辑 / 删除菜品。进详情自动读取菜品与该菜评价；「编辑」打开与列表同一个表单弹窗、「删除」经二次确认级联删评价；单条评价可抽屉查看详情（配图、正文、时间、显隐态）。

## UI

> 📐 页面 UI 设计稿已拆出 → [web-菜品详情查看.md（docs/ui）](../ui/web-菜品详情查看.md)（**UI 口径以该文件为唯一真源**）
...
> ⚠️ **Web 一律走 `/admin/**`，不得调用公开端点 `GET /dishes/{id}`**：公开端点只返回 `status='on'` 的菜品，**下架菜取不到名**。
