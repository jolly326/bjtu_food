...
> **UI 段为指针**：各页面的 UI 设计稿统一维护在 [`docs/ui/`](../ui/)（与功能文档**同名一一对应**），本目录五段中 `## UI` 位置仅保留指向该文件的指针；**UI 设计口径以 `docs/ui/` 为唯一真源**，功能流程 / 接口 / 字段 / 数据口径仍以本目录为准。
...
| 编号 | 功能 | 文档 | 鉴权 | 状态 |
|---|---|---|---|---|
| A-01 | 微信静默登录 / 游客态 | [client-微信静默登录与游客态.md](./client-微信静默登录与游客态.md) | 🔓 | ✅ **已完成** |
