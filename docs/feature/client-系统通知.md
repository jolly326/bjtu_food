...
接收**管理员处理反馈/举报、菜品纠错后的回执**（仅已认证用户）。现登记两种通知类型：`feedback_handle`=反馈处理回执、`correction_handle`=菜品信息纠错回执（采纳/拒绝均投递）；端上对未知类型容错。管理员处理反馈或菜品纠错时投递，正文含回复内容（不采纳时含不采纳原因）。游客提交的反馈/纠错不投递通知（不阻塞处理）。进页拉取通知列表（默认全部，可筛未读），点开单条即标记已读，支持一键「全部已读」（幂等，返回本次置读条数）；无通知时展示空态。
...
| GET | `/my/notifications/unread-count` | 同上 | 未读总数（驱动红点）；轻量性能端点，等价于 `GET /my/notifications?isRead=0` 的 `total`，避免为红点拉回整页 records |
...
|---|---|---|
| `id` | number | 通知 ID |
| `type` | string | 通知类型键：现登记 `feedback_handle`=反馈处理回执、`correction_handle`=菜品信息纠错回执（采纳/拒绝均投递）；端上对未登记/历史类型按容错处理（展示、不跳转、不崩溃） |
| `title` | string | 通知标题 |
| `content` | string | 通知正文（回执内容；不采纳时含不采纳原因） |
| `relatedId` | number | 关联对象 ID（`feedback_handle` 场景 = **反馈 ID**）；为 deep-link 预留锚点，当前无落地页、不参与路由（端上仅展示、不跳转） |
