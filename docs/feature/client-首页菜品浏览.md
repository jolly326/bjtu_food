...
> **2026-09-21 首页 UI 重构决议（目标态）**：本节的「初始态 / 吸顶态」结构与卡片排版按新设计规格执行；`app` 主色同时由暖砖红改为**橙色**（全站 token 层，见 G 项）。当前实现为「暖砖红头部（仅搜索框）+ 食堂/价格筛选行 + 单列/双列内容区」，与本节目标态存在差异，随 change 落地。

### 页面两态（同一页面滚动切换）

**① 初始态（未上滑，Banner 完整展示）**

1. 左上角页面标题：**知行食记**（黑色粗体大号）；
2. **Banner**：浅橙柔和渐变背景，主标题「今日推荐」（粗体深棕大字号）、副标题「发现食堂里的美味搭配」，右侧手绘美食插画（面条 / 饮品）。**本期为静态运营位（无后端取数）**；如需动态推荐另立 change；
3. Banner 下方**通栏搜索组件**：圆角白底输入框、灰色占位「搜索菜品、食堂、套餐」、左侧放大镜图标；**搜索框右侧对齐文字按钮「筛选」**（带下拉小箭头）→ 打开既有食堂 / 价格筛选面板（筛选维度不变，与新增的大类标签栏并列）；
4. **横向大类标签栏**（详见下）——横向可滑动，选中项下方橙色短下划线高亮，未选中为黑色常规字；
5. **菜品内容区**：双列等宽卡片网格，卡间水平 / 垂直间距统一，**禁止三列**；所有卡片完整展示、底部不半截截断。
...
1. **主色 = 橙色**：用于价格、TabBar 激活态、标签下划线、主按钮等；**全站替换**（小程序 + Web 管理端），非仅首页；具体色值需验证与页面底（浅米白）/ 卡片底（白）的对比度（正文级 ≥4.5:1、大字与图形 ≥3:1）；
2. 页面背景：**浅米白 → 淡橙顶部渐变**；
...
> ⚠️ **代码同步状态**：`list` 目前仍由 `PageResult.getList()` 输出；删除随契约变更落地，且该字段为**全部列表接口共用**（`/dishes`、`/my/reviews`、`/my/notifications`、`/admin/*` 等），需一次全端切换。
...
> ⚠️ 注意区分：§7.31 保留的是 `canteen.location` **数据库列**（留档用），与「是否作为 `GET /canteens` 公开出参」是两件事。**代码同步状态**：字段仍在 `CanteenInfoVO` 中，删除随契约变更落地。
...
| G6 | **Banner 为静态运营位（无后端取数、无推荐算法）——确认；插画本期用占位实现** | 占位 = **首页私有组件 `pages/home/HomeBannerArt.vue`**（内联 SVG + `image` data-uri，与 `IconSvg` 同一机制：微信小程序无原生 `<svg>`，故走 data-uri；线性风格，面条 + 饮品意象）。正式资产到位时**只替换该组件内容**，不触碰首页结构；按 PR-05 登记占位实现，不留零消费资产 |
