...
1. **顶部通栏 Banner**：左右无间隙、占据页面最顶部区域（含状态栏背后），作为「知行食记」标题的**背景**（标题叠加左上角，黑色粗体大号）；文案 = 主标题「今日推荐」（粗体深棕大字号）+ 副标题「发现食堂里的美味搭配」；背景图用 `<image>` **单独加载**（URL 由页面常量 `BANNER_BG_SRC` 承载，正式资产到位只改该常量；`aspectFill` 铺满；未配置 / 加载失败**回退渐变底** `--color-primary-soft → --bg-page-grad-to`，**不做任何组件手绘**）。**静态运营位**，动态推荐另立 change；
...
**向上滑动（`scrollTop` 增大）**：Banner 跟随内容同步上移、逐步移出屏幕，搜索框 / 标签栏随之上移；`scrollTop ≥ 阈值` 时**瞬间切换吸顶态**。

**吸顶态**（`scrollTop ≥ 阈值`）：

1. Banner 整块完全消失（视口内不再渲染）；
...
> **图片占位策略（本期所有图片先占位，禁止无限空转加载）**：① 菜品图——失败立即回退「餐具」占位图标；② Banner 背景图——未配置 / 失败回退渐变底；③ 列表请求失败 → 失败重试块（`RetryBlock`）。**预览前置**：后端（含 MySQL）部署在**云端**，本地无需起库；http 域名需在开发者工具勾选「不校验合法域名」。
