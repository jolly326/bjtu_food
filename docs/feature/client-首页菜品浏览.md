...
### 5. 滚动交互（初始态 ↔ 吸顶态，垂直滑动触发）

**触发**：仅**垂直滚动**切换状态；横向拖动标签栏不影响页面滚动与吸顶。两态：【初始态】、【吸顶态】。

**初始态**（滚动偏移量 ≤ 阈值）自上而下完整渲染：① Banner（标题 + 今日推荐主副标题 + 背景图）→ ② 搜索框 → ③ 大类标签栏 → ④ 双列卡片列表；TabBar 常驻底部。
**滚动阈值 = Banner 区块可视高度**（Banner 完全移出屏幕的滚动距离，如 Banner 高 320px 则 `scrollTop < 320px` 保持初始态）。

**向上滑动（`scrollTop` 增大）**：Banner **随内容一同自然上滑**（观感同菜品详情页顶部大图——整卡刚体上移）；当 Banner **下边缘抵达吸顶头部底缘**时**停止上滑**——定格为吸顶头部的背景层，此后菜品列表继续在头部下方滚动。

> ✅ **代码已落地（2026-09-21，change `home-scroll-interaction`）**：本节全部条款已实现——Banner 图层随内容 1:1 刚体上移、下缘抵达头部底缘（onReady 实测头部高）后定格；点标签不回顶；下拉刷新强制回顶；横向拖动不影响；猛滑无过渡。原「裁剪窗口折叠收起」与「筛选变更回顶（D6）」均已废止。

**吸顶态**（`scrollTop ≥ 阈值`）：

1. Banner **定格于吸顶头部背后**（下缘抵达头部底缘后停止移动，作为吸顶头部的背景层）；
2. 「知行食记标题 + 搜索框 + 标签栏」组合为**粘性吸顶容器**，固定锁定屏幕最顶部：内部结构不变（标题左上、搜索框、筛选按钮、横向标签栏全保留），标签栏仍可**左右拖动**、点击筛选正常，搜索 / 筛选功能完全可用；
3. 吸顶容器下方：菜品列表继续滚动，可上拉加载更多；
4. TabBar 依旧固定底部。

**向下滑动（`scrollTop` 减小）**：`scrollTop < 阈值` 时退出吸顶态切回初始态，Banner 重新完整出现，标题 / 搜索框 / 标签栏恢复跟随页面滚动。

**边界行为（必须处理）**：

1. **快速猛滑**：状态切换只判断 `scrollTop` 是否跨过阈值，**无过渡动画延迟**，跨阈瞬间切换；
2. **横向拖动标签栏**：只滚动标签内部横向内容，**不改变页面 `scrollTop`、不触发吸顶切换**；
3. **标签点击**：仅刷新菜品列表，**不重置页面滚动位置**（保持当前吸顶 / 初始态不变）；
4. **下拉刷新**：强制重置滚动位置到顶部，回到【初始态，Banner 完整展示】；
5. **上拉加载更多**：不改变吸顶状态。

> ✅ **代码已落地（2026-09-21，change `home-scroll-interaction`）**：边界 3（筛选变更不重置滚动位置）与边界 4（下拉刷新强制回顶至初始态）均已实现；其余条款此前已落地。原「筛选变更一律回顶」约定（D6）随之废止。

**一句话精简版**：页面垂直向上滑动，当滚动距离 ≥ Banner 高度时，Banner 整体移出视口；「知行食记标题 + 搜索框 + 大类标签栏」整体固定在屏幕顶部保持吸顶，菜品列表继续滚动；向下回滚，滚动距离小于 Banner 高度阈值时取消吸顶，Banner 重新出现，标题、搜索框、标签栏恢复跟随页面滚动。横向拖动标签栏仅滚动标签，不触发页面吸顶切换。
