...
> **2026-09-21 首页 UI 重构（已落地，change `home-ui-refresh`）**：本节即当前实现口径（`build:mp-weixin` EXIT 0、产物逐项核验）；主色已由暖砖红改为**橙色两档**（定稿取值见「视觉规范」与 G4）。
...
| 区块 | 内容 | 滚动行为 |
|---|---|---|
| ① 顶部 Banner | 整块通栏背景（规格见 2.①） | 上滑随滚动 **1:1 折叠收起**（与手势逐像素等价、无空带无跳变；**禁用 `position: sticky`**） |
| ② 搜索组件 | 输入框 + 右侧「筛选」按钮 | **常驻吸顶**，任何滚动位置不消失 |
| ③ 大类标签栏 | 横向单选标签（规格见 2.③） | 紧随搜索组件**常驻吸顶** |
| ④ 菜品网格 | 双列瀑布流卡片 | 页面唯一滚动内容 |

- **初始态**：Banner 完整展开，「知行食记」标题叠加于 Banner 左上角（Banner 即标题背景）；
- **吸顶态**（上滑后）：「标题 + 完整搜索框 + 标签栏」整体固定于视口顶部，**Banner 完全不可见**；吸顶态下标签栏仍可横滑、点选，搜索 / 筛选功能保持可用；
- **回滚**：Banner 对称还原，退出吸顶；右上角微信原生胶囊**任何状态下不被遮挡**。

### 2. 组件规格

**① 顶部 Banner（静态运营位，无后端取数）**

- **左右无左右边距，占据页面最顶部区域**（含状态栏背后），作为「知行食记」标题的**背景**——标题叠加绘制于 Banner 左上角（黑色粗体大号）；
- 文案层：主标题「今日推荐」（粗体深棕大字号）+ 副标题「发现食堂里的美味搭配」；
- 背景：`<image>` **单独加载**（`aspectFill` 铺满；URL 由页面常量 `BANNER_BG_SRC` 承载，正式资产到位后只改该常量），未配置 / 加载失败**回退渐变底** `--color-primary-soft → --bg-page-grad-to`，**不做任何组件手绘**（原占位插画组件已删除）；
- 如需动态「今日推荐」另立 change。

**② 搜索组件（通栏）**

- 圆角白底输入框，灰色占位「搜索菜品、食堂、套餐」，左侧放大镜图标；
- 右侧「筛选」文字按钮（下拉小箭头）→ 展开食堂 / 价格筛选面板（白底、锚定按钮正下方、点击面板外关闭），与标签栏维度**可叠加**。

**③ 横向大类标签栏**

- 数据源：`GET /dishes/meal-types`（`[{ key, label, order }]`）——文案、顺序、子集、增删**全由后端决定，端上一个不写死**；首项「全部」= 不传 `mealType`，由端上固定渲染；
- **空类自动隐藏**：后端只下发「当前有在售菜品」的大类，某类暂时空了自动隐藏、有菜自动出现，端上零改动；
- 横向可滑动；**UI 为单选模式**，同一时间仅选中一个大类标签（2026-09-21 走查勘误：原「大类之间菜品互斥」易误读为菜品数据层面互斥——菜品归属大类由后端定义，此处描述的是**标签交互**）；选中项下方**橙色短下划线**高亮，未选中为黑色常规字；
- 点击切换 → 列表刷新并重置分页；v1 **不显示计数**（省聚合查询）。

**④ 菜品卡片（四段固定排版，自上而下）**

| 段 | 内容 | 样式 |
|---|---|---|
| 1 | 菜品实拍图 | 圆角与卡片一致，高约占卡片 **52%**（3:2 容器）；无图 / 加载失败 → 「餐具」占位图标（禁止裂图） |
| 2 | 菜品名称 | **黑色加粗**，本卡片最大字号（超 2 行省略） |
| 3 | `食堂名称 \| 档口名称` | **浅灰常规小字，纯文字**——无图标、无任何彩色背景标签 |
| 4 | 左：**黄色实心五角星** + 数字评分；右：**价格（橙色突出）** | 同一行；星形尺寸对齐评分文本（30rpx，光学补偿 SVG 留白） |

- ~~`月售XXX份`~~ **不引入**（2026-09-21 决议：项目无订单 / 销量数据源）；
- 列表末尾**无引导 / 补录类组件**——原「想吃啥没找到？告诉我们」贡献卡已于 2026-09-21 走查后**删除**（连同「清除筛选」次级动作与首页贡献入口），首页内容流以菜品网格 + 触底提示收尾。

### 3. 视觉规范

1. **主色 = 橙色（定稿，G4）**：两档——填充档 `--color-primary` `#C2410C`（主按钮 / 图标 / TabBar 激活图标，兼作承载白字的底色）、文字档 `--color-primary-text` `#B93A0A`（价格 / TabBar 激活文字 / 选中态文字）；标签下划线按走查反馈取 `--color-primary-bright` `#EA580C`（图形级 ≥3:1）；**全站替换**（小程序 + Web 管理端）；
2. 页面背景：**浅米白 → 淡橙顶部渐变**（`--bg-page-grad-from` `#FFF9F3` → `--bg-page-grad-to` `#FFEFE0`）；
3. 文字层级：标题 / 菜名 = 黑色粗体；正文 = 黑色常规；次要信息（食堂档口、时间等）= 浅灰；
4. 星色 = `--color-star` **黄色**（`#F5A623`，星色不随主色换肤）。

### 4. 底部 TabBar

固定 2 项：**首页**（橙色图标 + 橙色文字，激活）｜**我的**（灰色）；导航栏下方预留系统手势条安全区。（TabBar 仅 2 页为既有现状，无需改动。）
...
> **图片占位策略（2026-09-21 走查定稿：本期所有图片先占位，禁止无限空转加载）**：① **菜品图**——加载中淡入、加载失败 → 立即回退「餐具」图标占位块（禁止裂图与无限等待）；② **Banner 背景图**——未配置 / 加载失败 → 回退渐变底（见 2.①）；③ 列表请求失败 → 显示**失败重试块**（`RetryBlock`），不静默空转。**预览前置条件**：后端（含 MySQL）已部署在**云端**，本地无需起库；预览仅要求小程序可访问后端接口——若接口 / 图片域名为 http 或未登记合法域名，在开发者工具勾选「不校验合法域名」即可。
