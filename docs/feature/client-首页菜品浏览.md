...
> ⚠️ **代码同步状态（1 处，本节先行定稿）**：现行实现为「头部行（标题 / 搜索 / 标签栏）**恒定不位移** + 仅 Banner 图层上移定格」，与本节「初始态四者属页面正常流、滑动时刚体同步上移」不一致——**待按本节调整**。其余条款（阈值定义、Banner 下缘定格、点标签不回顶、下拉刷新回顶、横向拖动、猛滑无过渡）均已落地（change `home-scroll-interaction`）；原「裁剪窗口折叠收起」与「筛选变更回顶（D6）」均已废止。
