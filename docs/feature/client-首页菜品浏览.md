...
> 返回：[功能总览](./README.md)
> 最后更新：**2026-09-22**（本日变更：Banner 接口化 16:10 多图轮播；首页结构改「Banner 整块正常流 + 搜索区/大类标签栏吸顶」；**食堂 / 价格筛选全量下线**并删除食堂字典端点；列表出参拆出 `DishListItemVO` 8 字段；**吸顶位移改原生粘性定位 + 标签栏上下间距二次收口**——见文末「与当前代码的差异」L 项）
...
| 4 | 搜索区结构 | 旧实现为「单颗搜索框（右侧筛选按钮）」 | **已落地**：左搜索胶囊 + 右独立「搜索」按钮（同高、间距 `--spacing-sm`、同为 `--radius-pill`，按钮填充档主色 + 白字）；纵向间距按 UI 文档 §1.2 —— Banner→搜索区 `--spacing-lg`(16px)、搜索区→标签文字 ≈12px（标签行内偏置）、标签栏→网格 视觉 ≈16px（容器下 padding + 标签行余量，**不叠加**） |
...
| 8 | 顶部表面过渡（透明 ↔ 不透明） | 原为硬切换：标题带永久透明（网格会透到标题后）、容器常驻切片 | **已落地方案 C「纱式淡出」**：标题带改为**与页面底同源的渐变切片**按 `scrollTop` 在 `[H_b − 60, H_b]` 由 0 → 1 渐显（只柔化 Banner、不淡化标题）；容器改为**恒为同源切片**（基准 = 容器顶边页面坐标，2026-09-22 二次修订，去掉了「透明 ↔ 不透明」硬切）；完整口径见 UI 文档 §1.3 |
...
> **L 项（吸顶位移改「原生粘性定位」+ 标签栏纵向间距二次收口，2026-09-22）已落地**：① 吸顶容器由「滚动容器**外**的 `position: fixed` + `@scroll` 每帧 `setData` 下发 `transform`」改为**滚动容器内部内容流里的 `position: sticky` + `top` = 固定标题带下沿**——位移由渲染层原生滚动驱动、不经逻辑层，消除上滑时「位置像临时算出来的」滞后闪现（连带删除位移量 computed、`onReady` 高度实测、`.home-sticky-hold` 站位块与兜底常量）；② 容器表面改为**恒为「与页面底同源」的渐变切片**（切片基准 = 容器顶边在页面坐标中的位置），不再做「透明 ↔ 不透明」硬切；③ 标签栏纵向间距二次收口为**上行 ≈12px / 下行 ≈16px**（旧口径 20 / 24px，行外 padding 不再叠加）。**本批不涉及接口 / 字段 / 库表**。**UI 侧同步项见 [docs/ui/client-首页菜品浏览.md](../ui/client-首页菜品浏览.md) §1.2 / §1.3 / §2 / §5 / §6**；`project_spec.md` 见 §7.38（同时取代 §7.34 第 ① 条「不用 `position: sticky`」的实现口径）。
