...
1. **顶部通栏 Banner**：左右无间隙、占据页面最顶部区域（含状态栏背后），作为「知行食记」标题的**背景**（标题叠加左上角，黑色粗体大号）；文案 = 主标题「今日推荐」（粗体深棕大字号）+ 副标题「发现食堂里的美味搭配」；背景图用 `<image>` **单独加载**（URL 由页面常量 `BANNER_BG_SRC` 承载，正式资产到位只改该常量；`aspectFill` 铺满；未配置 / 加载失败 → **占位图**：灰底 + 居中「餐具」图标 + 「Banner 占位图」说明，与菜品卡占位**同款**——禁止只留纯色底）。**静态运营位**，动态推荐另立 change；
...
**向上滑动（`scrollTop` 增大）**：Banner **随内容一同自然上滑**（观感同菜品详情页顶部大图——整卡刚体上移，非原位折叠收缩）；当 Banner **下边缘抵达吸顶头部底缘**时**停止上滑**——整块被头部覆盖，视口内不再可见；此后菜品列表继续在头部下方滚动。

> ⚠️ **代码待同步（1 处）**：现行实现为「裁剪窗口 1:1 折叠收起」（观感为 Banner 原位收缩），将按本节替换为「随内容自然上滑 + 下缘定格」；其余条款（点标签不回顶 / 下拉刷新回顶 / 横向拖动 / 猛滑）均已落地。

**吸顶态**（`scrollTop ≥ 阈值`）：

1. Banner **定格于吸顶头部背后**（下缘已抵达头部底缘后停止移动，视口内不再可见）；
