...
- **2026-08-03 用户裁定（本轮精简）**：① 弹层顶部边界 `topOffset` 传入（档口页传 176rpx），弹层不越过页面 Header 底部；② 头部**无「菜品详情」标题**，直接放返回按钮（arrow-left 关闭）；③ **图片贴屏幕左右边缘**（`sheet-body` 顶部 padding 0，卡片自带左右 margin）；④ hero 卡精简为 **名称 +「信息有误？」/ 价格 / 标徽 / 评分 / 一句话简介**，**删除「菜品信息」合并卡**（位置/营业/辣度/分量/供应时段均移除）；⑤ 反馈入口从底部 `.apply-link` 上移至 hero 名称行右侧「信息有误？」。

## 1. 页面定位
菜品详情（现为底部弹层 `DishDetailSheet`）：图集（贴边）+ hero 卡（名称 +「信息有误？」/ 价格/促销 / 标徽 / 评分 / 一句话简介）+ 用户评价预览；底部固定操作栏（喜欢/写评价/去档口）；分享面板。弹层顶部不越过页面 Header 底部。

## 2. 布局结构
- 弹层头部：无标题，左侧返回按钮（arrow-left，tap 关闭）+ 右侧 share 图标。
- 滚动区 `scroll-view`（refresher）。
- 三态：
  1. 加载中：`.dish-skeleton`（swiper/两张卡片骨架，pulse 动画）。
  2. 正常：`ImageSwiper`（贴左右边缘）→ `CardSection` hero（名称 +「信息有误？」/ 价格 / 标徽 / 评分 / 一句话简介）→ `CardSection`(用户评价 SectionTitle+前 3 ReviewItem) → 关联动态（低优先级）。
...
- 信息架构：hero 单卡承载名称/价格/标徽/评分/简介 +「信息有误？」；详情属性（辣度/分量/营业/供应时段）**不再展示**（用户裁定去「菜品信息」卡）。

## 3. 核心组件与用法
- `ImageSwiper`：菜品图集（贴左右边缘）。
- `TagLabel`：标徽（口味标签）。
- `CardSection` / `SectionTitle`：hero 卡 / 评价区标题（accent 条）。
- `ReviewItem`：评价项（前 3）。
- `AppButton`：写评价 / 去档口（主+outline 次级）。
- `ApplySheet`：申请下架/纠错弹层（spring 0.8/0.3 + ic-close + reduced-motion；入口为 hero 名称行右侧「信息有误？」）。
- 分享面板：自绘 `.share-sheet` + `.sheet-mask`（下拉关闭手势，阈值 120px，从底部进入）。
- `IconSvg`：`share`(顶/分享项)、`arrow-left`(返回)、`clock`(限时)、`star-filled`(评分)、`heart`/`heart-filled`(喜欢)、`close`(分享关闭)——均注册 key。
...
- 「信息有误？」：hero 名称行最右弱链接（`--font-aux` `--text-tertiary`），点击开 ApplySheet（原底部 `.apply-link` 已移除）。
- **位置行**：食堂 › 楼层 › 档口 › 窗口，点击跳档口详情（`goToStall`）；**从档口详情页打开时（`hideLocation`）仅静态展示、无跳转箭头、不可点击**——避免 菜品→档口→菜品 循环跳转（2026-08-03 用户裁定）。
...
- 返回：弹层头部 arrow-left 点击关闭（`requestClose`）。
- 进场：区块 `enter-up` 错峰（30–80ms），仅 transform/opacity，无 scale(0)。

## 6. 一致性红线自检（project_spec §4.9）
- ①图标 IconSvg：✅ share/arrow-left/clock/star-filled/heart/heart-filled/close 均 IconSvg；无 emoji。
...
- ⑤Sheet 规范：✅ ApplySheet 套通用弹层；分享面板自绘但遵循 spring(0.8/0.3 抽屉缓动)+ic-close+下拉手势+reduced-motion 降级，同源规范；弹层顶部边界用 `topOffset` 不越过页面 Header。
- ⑥按压 0.97：✅ 返回按钮/fav/分享项/action-bar 均 `scale(var(--press-scale))`；「信息有误？」弱化为文字（无按钮态）。
- ⑦颜色 token：✅ 无裸 hex（促销/限时 `--color-error`/`--color-hot`；喜欢 `--color-like`；评分 `--color-star`）。
- ⑧SectionTitle：✅ 仅用户评价区用 SectionTitle；hero 卡不用大标题（2026-08-03 用户裁定），无「菜品信息」卡。
- ⑨底部避让：✅ action-bar 与 scroll-wrap 用 `--action-bar-height` + 安全区；分享面板含安全区 padding。

## 7. 待定 / 可编辑的设计方案
> 以下区块由产品或设计在此直接编辑，用于和开发者同步 UI 变更意向。
- 待讨论项：**「信息有误？」链接用 `--text-tertiary`/`--font-aux`**，建议验证其对比度与最小字号(§8⑥⑯)；**价格/优惠价/评分数字 tabular-nums 已落地**（DishDetailSheet.vue 各数字类，见 §8⑮）
- 计划调整：采纳
...
| 6 | 正文对比度 ≥4.5:1，亮暗双测 | 部分 | token 满足；「信息有误？」`--text-tertiary` 对比建议验证（见 §8⑯） |
...
| 15 | 数字 tabular-nums | 合规 | 价格 `.price-text` / 促销价 `.promo-price` / 划线价 `.origin-price` / 评分值 `.star-num-text` 均已 `font-variant-numeric: tabular-nums`（DishDetailSheet.vue） |
| 16 | 正文≥16px(32rpx)，行高1.5–1.75，4/8pt 节奏 | 部分 | 「信息有误？」`--font-aux`(22rpx) 仅作辅助弱链接；主文 ≥32rpx；4/8pt 间距 |
| 17 | 每屏一个主 CTA，破坏性弱化隔离 | 合规 | 主「去档口」+ outline「写评价」；「信息有误？」弱化为 hero 名称行小文字链接（非大板块，符合信息架构） |
...
- [ ] 375px：图集贴边不溢出；hero 卡与评价卡不溢出；底部操作栏三按钮不挤。
- [ ] 弹层边界：顶部不越过页面 Header 底部（`topOffset` 生效，档口页 176rpx）。
- [ ] reduced-motion：ApplySheet/分享面板交叉淡入；骨架 pulse 停。
- [ ] 动态字号：菜名/价格/简介不截断。
- [ ] 暗色对比：「信息有误？」`--text-tertiary` 对比（建议验证）；价格/评分文字达标。
- [ ] 44pt：返回/fav/写评价/去档口/share ≥44pt。
