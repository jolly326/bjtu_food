...
- **颜色全走语义 token（禁裸 hex）**：所有颜色（含限时 / 促销 / 热门等标签底色、`IconSvg` 的 `color` 属性、文字色、边框色、背景色）必须引用语义 token（如 `var(--color-hot)` / `var(--color-promo)` / `var(--color-primary)` 等）；**禁止**在模板 / 组件样式中写裸 hex（如 `#FF6B6B` / `#FFB400`）。原生 API 不接受 `var()` 的例外（如 `<swiper indicator-active-color>`、`confirmColor` 等）**须在 `frontend/src/uni.scss` 注释登记**（注明对应 token 名与用途），登记后不作为红线违规。当前违规点见 §9.2 / fix-plan（如 `pages/pages-detail/dish.vue` 限时促销标签裸 hex）。
